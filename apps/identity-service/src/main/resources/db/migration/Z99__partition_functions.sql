...
--
-- Canonical partition-maintenance functions for the `identity` schema.
...
--   * partman.ensure_partitions('identity.identity_claim_history', 12)
--   * partman.ensure_partitions('identity.role_assignment_history', 12)
-- pg_cron schedules run at 02:00 UTC daily.
...
SELECT cron.unschedule('identity.partition.identity_claim_history.ensure');
SELECT cron.schedule(
    'identity.partition.identity_claim_history.ensure',
    '0 2 * * *',
    $$ SELECT partman.ensure_partitions('identity.identity_claim_history'::REGCLASS, 12) $$);

SELECT cron.unschedule('identity.partition.role_assignment_history.ensure');
SELECT cron.schedule(
    'identity.partition.role_assignment_history.ensure',
    '0 2 * * *',
    $$ SELECT partman.ensure_partitions('identity.role_assignment_history'::REGCLASS, 12) $$);

SELECT cron.unschedule('identity.partition.identity_claim_history.drop_expired');
SELECT cron.schedule(
    'identity.partition.identity_claim_history.drop_expired',
    '0 3 * * 0',
    $$ SELECT partman.drop_expired_partitions(
         'identity.identity_claim_history'::REGCLASS,
         INTERVAL '3 years') $$);

SELECT cron.unschedule('identity.partition.role_assignment_history.drop_expired');
SELECT cron.schedule(
    'identity.partition.role_assignment_history.drop_expired',
    '0 3 * * 0',
    $$ SELECT partman.drop_expired_partitions(
         'identity.role_assignment_history'::REGCLASS,
         INTERVAL '3 years') $$);
