...
--
-- pricing-service partitions the time-series tables by month:
--   * pricing.quote_cache : expires_at (monthly, 7-day retention)
...
BEGIN
    ...
    v_time_col := CASE v_table
        WHEN 'quote_cache' THEN 'expires_at'
        ELSE NULL
    END;
...
CREATE OR REPLACE FUNCTION pricing.reject_update() RETURNS TRIGGER AS $$
BEGIN
    RAISE EXCEPTION 'pricing.% is append-only: UPDATE blocked', TG_TABLE_NAME
        USING ERRCODE = 'integrity_constraint_violation';
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION pricing.reject_delete() RETURNS TRIGGER AS $$
BEGIN
    RAISE EXCEPTION 'pricing.% is append-only: DELETE blocked', TG_TABLE_NAME
        USING ERRCODE = 'integrity_constraint_violation';
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS rule_bindings_history_no_update ON pricing.rule_bindings_history;
CREATE TRIGGER rule_bindings_history_no_update
    BEFORE UPDATE ON pricing.rule_bindings_history
    FOR EACH ROW EXECUTE FUNCTION pricing.reject_update();

DROP TRIGGER IF EXISTS rule_bindings_history_no_delete ON pricing.rule_bindings_history;
CREATE TRIGGER rule_bindings_history_no_delete
    BEFORE DELETE ON pricing.rule_bindings_history
    FOR EACH ROW EXECUTE FUNCTION pricing.reject_delete();