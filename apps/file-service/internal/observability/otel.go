...
// deployment sets FILE_SERVICE_OTEL_EXPORTER_OTLP_ENDPOINT and gets the
// full OTLP-gRPC exporter. The function returns a shutdown hook that
// drains the SDK on process exit; main wires it via defer.
func Init(ctx context.Context, endpoint, service string) (shutdown func(context.Context) error, err error) {
	...
	slog.Default().Info("otel: OTLP endpoint configured but exporter wiring lands in a follow-up", slog.String("endpoint", endpoint), slog.String("service", service))
	_ = ctx
	return func(context.Context) error { return nil }, nil
}
