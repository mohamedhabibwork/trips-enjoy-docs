// Package db is the file-service PostgreSQL data layer. Connect() returns
// a pgxpool.Pool wired with search_path=file,public so every unqualified
// table lookup resolves against the file schema first. The production
// wiring also runs golang-migrate at boot (cmd/server/main.go); this
// scaffold defers that to make the binary runnable without a local DB.
package db
