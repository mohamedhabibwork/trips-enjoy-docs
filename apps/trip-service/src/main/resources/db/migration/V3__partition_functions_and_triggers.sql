...
--
-- trip-service partitions the trip_location_point table by month
-- (recorded_at). trip_state_history is a regular indexed table
-- (single UUID PK, occurred_at indexed separately) — no partitioning.
...
BEGIN
    ...
    v_time_col := CASE v_table
        WHEN 'trip_location_point' THEN 'recorded_at'
        ELSE NULL
    END;
...
CREATE OR REPLACE FUNCTION trip.reject_update() RETURNS TRIGGER AS $$
BEGIN
    RAISE EXCEPTION 'trip.% is append-only: UPDATE blocked', TG_TABLE_NAME
        USING ERRCODE = 'integrity_constraint_violation';
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION trip.reject_delete() RETURNS TRIGGER AS $$
BEGIN
    RAISE EXCEPTION 'trip.% is append-only: DELETE blocked', TG_TABLE_NAME
        USING ERRCODE = 'integrity_constraint_violation';
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trip_state_history_no_update ON trip.trip_state_history;
CREATE TRIGGER trip_state_history_no_update
    BEFORE UPDATE ON trip.trip_state_history
    FOR EACH ROW EXECUTE FUNCTION trip.reject_update();

DROP TRIGGER IF EXISTS trip_state_history_no_delete ON trip.trip_state_history;
CREATE TRIGGER trip_state_history_no_delete
    BEFORE DELETE ON trip.trip_state_history
    FOR EACH ROW EXECUTE FUNCTION trip.reject_delete();

DROP TRIGGER IF EXISTS trip_reward_reversal_no_update ON trip.trip_reward_reversal;
CREATE TRIGGER trip_reward_reversal_no_update
    BEFORE UPDATE ON trip.trip_reward_reversal
    FOR EACH ROW EXECUTE FUNCTION trip.reject_update();

DROP TRIGGER IF EXISTS trip_reward_reversal_no_delete ON trip.trip_reward_reversal;
CREATE TRIGGER trip_reward_reversal_no_delete
    BEFORE DELETE ON trip.trip_reward_reversal
    FOR EACH ROW EXECUTE FUNCTION trip.reject_delete();