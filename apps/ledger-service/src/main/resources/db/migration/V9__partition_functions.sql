...
--
-- Canonical partition-maintenance functions for the `ledger` schema.
...
-- at 03:00 UTC, 10-year retention, legal-hold aware.
...
SELECT cron.unschedule('ledger.partition.postings.ensure');
SELECT cron.schedule(
    'ledger.partition.postings.ensure',
    '0 2 * * *',
    $$ SELECT partman.ensure_partitions('ledger.postings'::REGCLASS, 12) $$);

SELECT cron.unschedule('ledger.partition.posting_entries.ensure');
SELECT cron.schedule(
    'ledger.partition.posting_entries.ensure',
    '0 2 * * *',
    $$ SELECT partman.ensure_partitions('ledger.posting_entries'::REGCLASS, 12) $$);

SELECT cron.unschedule('ledger.partition.postings.drop_expired');
SELECT cron.schedule(
    'ledger.partition.postings.drop_expired',
    '0 3 * * 0',
    $$ SELECT partman.drop_expired_partitions(
         'ledger.postings'::REGCLASS,
         INTERVAL '10 years') $$);

SELECT cron.unschedule('ledger.partition.posting_entries.drop_expired');
SELECT cron.schedule(
    'ledger.partition.posting_entries.drop_expired',
    '0 3 * * 0',
    $$ SELECT partman.drop_expired_partitions(
         'ledger.posting_entries'::REGCLASS,
         INTERVAL '10 years') $$);
