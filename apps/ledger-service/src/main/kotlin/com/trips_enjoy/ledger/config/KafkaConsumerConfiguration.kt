...
/**
 * Kafka consumer + error handling. Mirrors audit-service: manual ack,
 * read_committed isolation, exponential backoff (1 s → 8 s, 3 attempts),
 * poison events routed to `<topic>.dlq`.
 */
@Configuration
class KafkaConsumerConfiguration {

    @Bean
    fun ledgerKafkaListenerContainerFactory(
    ...
    private fun defaultErrorHandler(template: KafkaTemplate<String, Any>): DefaultErrorHandler {
        val recoverer = DeadLetterPublishingRecoverer(template) { record: ConsumerRecord<*, *>, _ ->
            val dlq = (record.topic() ?: "ledger.unknown") + ".dlq"
