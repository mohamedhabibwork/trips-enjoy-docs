...
/**
 * Spring Security resource-server config for ledger-service. Same shape as
 * audit-service / identity-service: stateless JWT bearer with realm + client
 * roles and OAuth2 scopes surfaced as `ROLE_<name>` / `SCOPE_<name>`.
 */
@Configuration
...
class SecurityConfiguration {
    ...
    @Bean
    fun jwtDecoder(@Value("\${ledger-service.keycloak.jwks-uri}") jwksUri: String): JwtDecoder =
