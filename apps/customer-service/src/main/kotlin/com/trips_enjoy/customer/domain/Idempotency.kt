...
/**
 * Idempotency-Key dedupe store per INTEGRATION.md §1.2 / SRS §15.
 * Caches the response status + body for 24h so a retry of the same
 * write returns the same answer without re-executing the underlying
 * transaction.
 */
@Entity
@Table(name = "idempotency", schema = "customer")
class Idempotency(
