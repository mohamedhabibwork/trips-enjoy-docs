...
/**
 * Kafka container + DLQ wiring for customer-service.
 *
 * Pattern (mirrors audit-service and configuration-service):
 * - Container factory: `customerKafkaListenerContainerFactory`
...
class KafkaConsumerConfiguration {
    @Bean
    fun customerKafkaListenerContainerFactory(
    ...
    private fun defaultErrorHandler(template: KafkaTemplate<String, Any>): DefaultErrorHandler {
        val recoverer =
            DeadLetterPublishingRecoverer(template) { record: ConsumerRecord<*, *>, _ ->
                val dlq = (record.topic() ?: "customer.unknown") + ".dlq"
