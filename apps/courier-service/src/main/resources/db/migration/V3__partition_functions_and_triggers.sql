...
--
-- courier-service partitions the time-series tables by month:
--   * courier.courier_rating_history : rated_at (monthly)
--
...
BEGIN
    ...
    v_time_col := CASE v_table
        WHEN 'courier_rating_history' THEN 'rated_at'
        ELSE NULL
    END;
...
CREATE OR REPLACE FUNCTION courier.reject_update() RETURNS TRIGGER AS $$
BEGIN
    RAISE EXCEPTION 'courier.% is append-only: UPDATE blocked', TG_TABLE_NAME
        USING ERRCODE = 'integrity_constraint_violation';
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION courier.reject_delete() RETURNS TRIGGER AS $$
BEGIN
    RAISE EXCEPTION 'courier.% is append-only: DELETE blocked', TG_TABLE_NAME
        USING ERRCODE = 'integrity_constraint_violation';
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS courier_rating_history_no_update ON courier.courier_rating_history;
CREATE TRIGGER courier_rating_history_no_update
    BEFORE UPDATE ON courier.courier_rating_history
    FOR EACH ROW EXECUTE FUNCTION courier.reject_update();

DROP TRIGGER IF EXISTS courier_rating_history_no_delete ON courier.courier_rating_history;
CREATE TRIGGER courier_rating_history_no_delete
    BEFORE DELETE ON courier.courier_rating_history
    FOR EACH ROW EXECUTE FUNCTION courier.reject_delete();

DROP TRIGGER IF EXISTS courier_audit_log_no_update ON courier.courier_audit_log;
CREATE TRIGGER courier_audit_log_no_update
    BEFORE UPDATE ON courier.courier_audit_log
    FOR EACH ROW EXECUTE FUNCTION courier.reject_update();

DROP TRIGGER IF EXISTS courier_audit_log_no_delete ON courier.courier_audit_log;
CREATE TRIGGER courier_audit_log_no_delete
    BEFORE DELETE ON courier.courier_audit_log
    FOR EACH ROW EXECUTE FUNCTION courier.reject_delete();