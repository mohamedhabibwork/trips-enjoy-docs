...
/**
 * The courier audit chain — every state-machine mutation writes one
 * row capturing the actor, the before/after snapshots, and a human-readable
 * reason. Mirrors `courier.courier_audit_log` per docs/services/courier-service/ERD.md §3.
 * Append-only (V3 trigger). Adds shift_* actions vs driver-service.
 */
@Entity
@Table(name = "courier_audit_log", schema = "courier")
class CourierAuditLog(
    @Id val id: UUID,
    ...
    companion object {
        ...
        const val ACTION_SHIFT_SCHEDULED = "shift_scheduled"
        const val ACTION_SHIFT_ACTIVATED = "shift_activated"
        const val ACTION_SHIFT_COMPLETED = "shift_completed"
        const val ACTION_SHIFT_CANCELLED = "shift_cancelled"
        ...
        val VALID_ACTIONS: Set<String> = setOf(
            ...
            ACTION_SHIFT_SCHEDULED, ACTION_SHIFT_ACTIVATED,
            ACTION_SHIFT_COMPLETED, ACTION_SHIFT_CANCELLED,
        )
    }
