...
--
-- admin-service partitions action_log by month (per the action_log ERD).
...
BEGIN
    ...
    v_time_col := CASE v_table
        WHEN 'action_log' THEN 'occurred_at'
        ELSE NULL
    END;
...
-- Append-only triggers for pricing_geo_config_history (admin-side mirror
-- of the pricing-service pattern).
CREATE OR REPLACE FUNCTION admin.reject_update() RETURNS TRIGGER AS $$
BEGIN
    RAISE EXCEPTION 'admin.% is append-only: UPDATE blocked', TG_TABLE_NAME
        USING ERRCODE = 'integrity_constraint_violation';
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION admin.reject_delete() RETURNS TRIGGER AS $$
BEGIN
    RAISE EXCEPTION 'admin.% is append-only: DELETE blocked', TG_TABLE_NAME
        USING ERRCODE = 'integrity_constraint_violation';
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS pricing_geo_config_history_no_update ON admin.pricing_geo_config_history;
CREATE TRIGGER pricing_geo_config_history_no_update
    BEFORE UPDATE ON admin.pricing_geo_config_history
    FOR EACH ROW EXECUTE FUNCTION admin.reject_update();

DROP TRIGGER IF EXISTS pricing_geo_config_history_no_delete ON admin.pricing_geo_config_history;
CREATE TRIGGER pricing_geo_config_history_no_delete
    BEFORE DELETE ON admin.pricing_geo_config_history
    FOR EACH ROW EXECUTE FUNCTION admin.reject_delete();