...
/**
 * Food-order-service outbox row — Phase B of the platform-DRY initiative
 * (ADR-0028): the local entity persists into the canonical 11-column
 * outbox shape on top of the existing `food_order.outbox` table.
 *
 ...
 * created_by, updated_by) live alongside them in the same table so the
 * existing constructor contract stays stable for callers.
 *
 ...
 */
@Entity
@Table(name = "outbox", schema = "food_order")
class OutboxEvent(
    @Id val id: UUID,
    ...
    fun markPublished(at: Instant) { publishedAt = at }
