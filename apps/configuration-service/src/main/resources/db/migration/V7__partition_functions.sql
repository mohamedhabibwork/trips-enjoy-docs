...
--
-- Canonical partition-maintenance functions for the `configuration` schema.
...
--   * partman.ensure_partitions('configuration.versions'::REGCLASS, 12) pre-
--     creates + verifies the next 12 monthly children.
--   * partman.ensure_partitions('configuration.audit_log'::REGCLASS, 12)
--     does the same for the audit log.
...
-- ----------------------------------------------------------------------------
-- drop_expired_partitions (configuration has no mixed retention today, so
-- the retention_class_filter is left NULL).
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- ----------------------------------------------------------------------------
-- pg_cron schedules — wrapped in DO so the migration is portable to clusters
-- without the pg_cron extension installed (e.g. local Testcontainers Postgres).
-- Production deployments that DO install pg_cron will have these schedules
-- registered; deployments without pg_cron will log a NOTICE and continue.
-- The Spring wrapper PartitionMaintenanceJob (see application/) is the
-- authoritative maintenance path.
-- ----------------------------------------------------------------------------
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
        PERFORM cron.unschedule('configuration.partition.versions.ensure');
        PERFORM cron.schedule(
            'configuration.partition.versions.ensure',
            '0 2 * * *',
            $cron$ SELECT partman.ensure_partitions('configuration.versions'::REGCLASS, 12) $cron$);
        PERFORM cron.unschedule('configuration.partition.audit_log.ensure');
        PERFORM cron.schedule(
            'configuration.partition.audit_log.ensure',
            '0 2 * * *',
            $cron$ SELECT partman.ensure_partitions('configuration.audit_log'::REGCLASS, 12) $cron$);
        PERFORM cron.unschedule('configuration.partition.versions.drop_expired');
        PERFORM cron.schedule(
            'configuration.partition.versions.drop_expired',
            '0 3 * * 0',
            $cron$ SELECT partman.drop_expired_partitions('configuration.versions'::REGCLASS, INTERVAL '1 year') $cron$);
        PERFORM cron.unschedule('configuration.partition.audit_log.drop_expired');
        PERFORM cron.schedule(
            'configuration.partition.audit_log.drop_expired',
            '0 3 * * 0',
            $cron$ SELECT partman.drop_expired_partitions('configuration.audit_log'::REGCLASS, INTERVAL '1 year') $cron$);
    ELSE
