 ...
 * `published_at`. On failure, increments `attempts` and stores the error;
 * the third failure routes the event to `<topic>.dlq` via Spring Kafka's
 * configured error handler.
 *
 * Mirrors audit-service/OutboxPublisher.
 */
@Component
class OutboxPublisher(
    ...
    @Scheduled(fixedDelayString = "\${configuration-service.outbox.publish-interval-ms:1000}")
    @Transactional
    fun publishPending() {
        ...
        for (event in pending) {
            try {
                // Partition key: topic+payload-based routing. For our 4
                // topics, the documentId is encoded in the payload so it's
                // a reasonable key.
                ...
                if (event.attempts >= 3) {
                    // Final attempt failed; route to DLQ via the broker.
                    try {
