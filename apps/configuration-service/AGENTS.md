...
matching documentation in `../../docs/services/configuration-service/` before
changes.

- Keep controllers, application services, persistence, integrations, and
  configuration separated under `src/`.
- Validate configuration scope, rollout, and authorization rules at the
  boundary; do not expose secret values through APIs, logs, or events.
