...
matching documentation in `../../docs/services/payment-service/` before
changes.

- Keep controllers, application services, persistence, integrations, and
  configuration separated under `src/`.
- Treat payment operations as financial and PCI-sensitive; preserve documented
  authorization, idempotency, saga, reconciliation, and audit semantics.
...
- Preserve correlation IDs, Kafka event versioning, and resilient downstream
  behavior required by the platform documentation.
