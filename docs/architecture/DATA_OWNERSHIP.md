...
The `Request` entity is a polymorphic shadow table owned per-service (per [ADR-0020](adrs/0020-polymorphic-request-id.md)). It binds the customer transaction (trip / order / delivery) to an orchestrator workflow instance via `workflow_process_id`. Each concrete aggregate (`trips`, `orders`, `deliveries`) carries a `request_id UUID NOT NULL UNIQUE` FK to its service's `requests` table. Cross-service references use `request_id` rather than the concrete aggregate's PK.

| Domain Entity | Owning Service | Database / Schema | Source of Truth | Referenced By | Request → Concrete Aggregate | Sync Method |
|---------------|----------------|-------------------|------------------|---------------|-------------|
...
| Ride request | `trip-service` (ride-request sub-aggregate) | `trip.ride_requests` | `trip-service` | `driver-service` (dispatch), `payment-service` (ride saga), `customer-service` (history), `trip-service` (history) | `request_id → trip.trips` (1:1 via `trip.trips.request_id`) | Events `ride.request.*.v1` |
| Trip | `trip-service` | `trip.trips` | `trip-service` | `payment-service` (ride saga + driver earnings), `driver-service` (incentives), `customer-service` (history), review projections (`trip-service` + `food-order-service` + `search-service`), `trip-service` (history + safety) | `request_id → trip.requests` (child; `trip.requests.id = trip.trips.request_id`) | Events `trip.*.v1` |
| Scheduled ride job | `trip-service` (scheduled sub-aggregate) | `trip.scheduled_rides` | `trip-service` | `trip-service` (ride-request) | `scheduled_rides.request_id → trip.requests` (same parent) | Events `scheduled_ride.due.v1` |
| Trip safety state (SOS, incident) | `trip-service` (safety sub-aggregate) | `trip.ride_safety` | `trip-service` | `notification-service`, `admin-service` (support module) | `ride_safety.request_id → trip.requests` (same parent) | REST + `ride.safety.*.v1` |
| Trip review (write side) | `trip-service` (trip-review projection) | `trip.trip_reviews` | `trip-service` | `driver-service` (rating), `courier-service` (rating), `reporting-service` | `trip_reviews.request_id → trip.requests` (same parent) | Events `review.submitted.v1` |
| Ride history (read model) | `trip-service` (history sub-aggregate) | `trip.ride_history_entries` | `trip-service` | `customer-service` app, `driver-service` app, `admin-service` | `history.request_id → trip.requests` (same parent) | Sync REST (read-only) |
...
| Food order | `food-order-service` | `food_order.orders` | `food-order-service` | `food-order-service` (queue), `courier-service` (dispatch + delivery), `payment-service` (food saga), `customer-service` (history), review projections | `request_id → food_order.requests` (child; `food_order.requests.id = food_order.orders.request_id`) | REST + `food.order.*.v1` |
...
| Food review (write side) | `food-order-service` (food-review projection) | `food_order.food_reviews` | `food-order-service` | `restaurant-service` (rating), `courier-service` (rating), `search-service` (discovery projections), `reporting-service` | `food_reviews.request_id → food_order.requests` (same parent) | Events `review.submitted.v1` |
| Delivery | `courier-service` (delivery sub-aggregate) | `courier.deliveries` | `courier-service` | `food-order-service` (read), `payment-service` (food saga + courier earnings), `customer-service` (history) | `request_id → courier.requests` (child; `courier.requests.id = courier.deliveries.request_id`) | REST + `delivery.*.v1` |
| Request (polymorphic shadow table) | owning service (`trip-service` / `food-order-service` / `courier-service`) | `<service>.requests` | owning service | All services that consume request-level state: `payment-service`, `ledger-service`, `notification-service`, `audit-service`, `reporting-service` | `request_id` IS the PK; each concrete aggregate (trips, orders, deliveries) has `request_id → requests.id` | Events `request.*.v1` |
| Payment intent | `payment-service` | `payment.intents` | `payment-service` | `payment-service` (ride saga + food saga + wallet), `customer-service` (history) | `request_id → <owning-service>.requests` (polymorphic FK; replaces `trip_id`, `order_id`, `food_order_id`) | REST + `payment.*.v1` |
...
| Notification delivery state | `notification-service` | `notification.deliveries` | `notification-service` | `admin-service` (support module, read) | `request_id → <owning-service>.requests` (polymorphic FK; replaces `trip_id`, `order_id`) | REST |
