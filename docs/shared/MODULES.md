...
| Frontend / BFF | web frontend reads OpenAPI artifact generated by `platform-spring-boot-api-docs`; BFF is **Kotlin + Spring Boot 4** (per the platform's "backend apps are Go, Kotlin, or Python only" rule — no Node.js on the backend); the Nuxt 3 SSR shell + the Vue 3 component library still need Node for the frontend tooling, but the BFF API is not Node-based |
