# driver-location-service — Implementation Plan

**Domain:** Ride-Hailing
**Tier:** 3
**Technology:** Go
**Criticality:** T1 (99.95%)
**DB Schema:** `driver_location`
**Cache:** Redis — last-known GlSet
**HPA:** CPU 70%, 4–12, p99 < 30ms
...
This PLAN.md is the source of truth for **how** `driver-location-service` is built. The 10-phase
...
- [ ] Create schema `driver_location`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `driver_location.outbox` and `driver_location.inbox` for reliable eventing
...
|-------|-------|---------|--------------|
| `driver.location.updated` | derived from name | see INTEGRATION.md | see INTEGRATION.md |

### Events Consumed
| Event | Producer | Handler |
|-------|----------|---------|
| `driver.availability.online` | see INTEGRATION.md | see INTEGRATION.md |
