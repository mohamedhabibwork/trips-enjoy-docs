...
**DB Schema:** `dispatch`
**Cache:** Redis — driver pool + active deal set
...
This PLAN.md is the source of truth for **how** `dispatch-service` is built. The 10-phase
...
- [ ] Create schema `dispatch`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `dispatch.outbox` and `dispatch.inbox` for reliable eventing
...
### Phase 5 — Caching
- [ ] Redis — driver pool + active deal set
- [ ] Push-invalidate on every write that affects the cache key
- [ ] Stampede protection on hot keys (single-flight)

### Phase 6 — External Integrations
- [ ] Sync dependencies: driver-availability-service, driver-location-service, ride-request-service
...
- Endpoints:
  - `GET /v1/dispatch/drivers/{id}/open-deals`
  - `POST /v1/dispatch/deals/{id}/bids`
  - `POST /v1/dispatch/deals/{id}/accept`
  - `POST /v1/dispatch/deals/{id}/bid/{bid_id}/reject`
- Produces:
  - `dispatch.deal.bid.submitted.v1`
  - `dispatch.deal.bid.expired.v1`
  - `dispatch.deal.accepted.v1`
  - `dispatch.deal.rejected.v1`
- Consumes:
  - `ride.deal.opened.v1`
...
| `driver-location-service` | per `INTEGRATION.md` | sync dependency | Yes |
| `ride-request-service` | per `INTEGRATION.md` | sync dependency | Yes |

### Events Published
| Event | Topic | Trigger | Key Consumers |
|-------|-------|---------|--------------|
| `dispatch.matched` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `dispatch.no_driver` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `dispatch.deal.bid.submitted.v1` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `dispatch.deal.bid.expired.v1` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `dispatch.deal.accepted.v1` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `dispatch.deal.rejected.v1` | derived from name | see INTEGRATION.md | see INTEGRATION.md |

### Events Consumed
| Event | Producer | Handler |
|-------|----------|---------|
| `ride.request.created` | see INTEGRATION.md | see INTEGRATION.md |
| `driver.location.updated` | see INTEGRATION.md | see INTEGRATION.md |
| `ride.deal.opened.v1` | see INTEGRATION.md | see INTEGRATION.md |
| `ride.deal.countered.v1` | see INTEGRATION.md | see INTEGRATION.md |
| `ride.deal.accepted.v1` | see INTEGRATION.md | see INTEGRATION.md |
| `ride.deal.rejected.v1` | see INTEGRATION.md | see INTEGRATION.md |
| `ride.deal.expired.v1` | see INTEGRATION.md | see INTEGRATION.md |
