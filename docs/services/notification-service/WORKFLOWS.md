...
- `notification.partition.deliveries.ensure` — `0 2 * * *` — calls
  `partman.ensure_partitions('notification.deliveries', 12)`.

The **fallback trigger** is the per-service Spring
`PartitionMaintenanceJob` (`apps/notification-service/src/main/kotlin/.../application/PartitionMaintenanceJob.kt`),
a `@Scheduled(cron = "0 0 2 * * *")` wrapper that acquires the
advisory lock and calls the same function. Leader-elected via
`pg_try_advisory_xact_lock(hashtext('notification'), hashtext('partition'))`.
...
sequenceDiagram
    ...
    CRON->>FN: SELECT partman.ensure_partitions('notification.deliveries', 12)
    FN->>PG: pg_advisory_xact_lock
    loop for each month in [-1, 12]
        FN->>PG: CREATE TABLE IF NOT EXISTS notification.deliveries_YYYY_MM PARTITION OF notification.deliveries
...
- After the function returns, the Spring wrapper hands the JSON to
  ...
  `notification.partition.maintained.v1` on topic
  `notification.partition.maintained` with `{schema: 'notification',
