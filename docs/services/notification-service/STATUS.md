# notification-service — Status Snapshot
...
> | Documentation completeness | filesystem scan (`docs/services/notification-service/`) |
...
| Bounded context | **Bounded Context**: *Orchestration of user-visible messages*. |
| Domain | — |
| Tier (deployment) | 0 (position 7 of 21; `DEPLOYMENT_ORDER.md` §2) |
| Criticality / SLO | — |
| Owner team | — |
...
|---|---|
| Status | ✅ Graduated |
| `apps/notification-service/` Dockerfile | ✅ present |
| `apps/notification-service/k8s/` (flat kustomize overlays) | ✅ present |
| `apps/notification-service/monitoring/` (ServiceMonitor + PrometheusRule) | ✅ present |
| Local test suite | (see §8.2 row above) |
| Implementation memory | see `uber-notification-service-implementation-*.md` in project memory index |
...
| `INTEGRATION.md` | ✅ mandatory | ✅ | 2026-08-14 |
| `WORKFLOWS.md` | ✅ mandatory | ✅ | 2026-08-12 |
| `TECH.md` | ✅ mandatory | ✅ | 2026-08-14 |
...
| Consumed events | 15 (INTEGRATION.md §4) |
...
| AuthZ | RBAC; admin role `notification-service.admin` per `TECH.md` §10 |
...
| Time-bounded alias | `platform-internal` realm `service-claims` scope mappers (per identity-service per-service claim contract); `notification-service.scopes` / `notification-service.level` / `notification-service.tenant` claims available |
...
| Field | Value |
|---|---|
| Plan header | Domain: — / Tier: — / DB Schema: — / Cache: — / HPA: — |
...
| Plan task total | (count of `T-notification-service`-prefixed rows in `PLAN.md`) |
...
- **Implementation memory** (graduates only): `uber-notification-service-implementation-<date>.md` (project memory index)
