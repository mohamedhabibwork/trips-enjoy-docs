# feature-flag-service — Technology Profile

> One-page technology reference for `feature-flag-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `feature_flag` (daily RANGE partitioned on `created_at`)
...
Redis — flag evaluation (TTL 30s, push-invalidate on flag change)
...
- **HPA signal**: CPU 60%, 2–8 replicas, p99 < 20ms
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile feature-flag up`
...
- **Key**: `audit.admin.feature_flag.v1`
...
|---|---|---|---|---|---|---|---|
| Flag definitions | ✓ | ✓ | ✓ | — | — | — | read |
| Flag evaluation log | ✓ | ✓ | read | — | — | — | read |
...
|---|---|---|---|
| `POST` | `/admin/v1/flags/{key}/toggle` | `feature_flag.admin` | Toggle a flag on/off (with audit + auto-rollout) |
| `POST` | `/admin/v1/flags/{key}/rollback` | `feature_flag.admin` | Roll a flag back to its previous value (with audit) |
| `GET` | `/admin/v1/flags/{key}/history` | `feature_flag.admin` | Full change history for a flag (with author + reason) |
...
- This service's per-service admin role `feature_flag.admin`.
...
emit `audit.admin.feature_flag.v1` (per §10.2) for each touched
record.
