# eta-routing-service — Technology Profile

> One-page technology reference for `eta-routing-service`. The platform-wide
...
- `go-redis/redis` v9 — ETA + route cache
- `resty` v2 — HTTP client for the map provider
- `prometheus/client_golang` v1.20+ — metrics

## 3. Data layer

- **Database**: PostgreSQL 18, schema `eta_routing` (cache only, monthly RANGE partitioned on `created_at`)
...
Redis — ETA + route cache (TTL 60s, surge-zone-aware invalidation)

## 5. External integrations

map provider (Google / Mapbox / HERE — pick one, abstract behind `MapProvider` interface)
...
- **HPA signal**: RPS, 3–40 replicas, p99 < 50ms
- **Pod resources**: requests/limits set per service (see `k8s/base/<service>/deployment.yaml`)

## 9. Local dev

- **Run**: `go run ./cmd/eta-routing`
- **Test**: `go test ./...`
- **Compose profile**: `docker compose --profile eta-routing up`
...
- **Key**: `audit.admin.eta_routing.v1`
...
|---|---|---|---|
| `POST` | `/admin/v1/cache/invalidate/{zone_id}` | `eta_routing.admin` | Invalidate the ETA + route cache for a specific zone (e.g. after a road closure) |
| `POST` | `/admin/v1/provider/rotate` | `platform.admin` | Rotate the map provider (Google → Mapbox) and warm the new cache |
...
- **Run with admin port**: `ADMIN_PORT=8081 go run ./cmd/eta-routing`
...
- This service's per-service admin role `eta_routing.admin`.
...
emit `audit.admin.eta_routing.v1` (per §10.2) for each touched
record.
