...
This PLAN.md is the source of truth for **how** `merchant-service` is built. The 10-phase
...
- [ ] Create schema `merchant`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `merchant.outbox` and `merchant.inbox` for reliable eventing
...
|-------|-------|---------|--------------|
| `merchant.created` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `merchant.approved` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `merchant.suspended` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
