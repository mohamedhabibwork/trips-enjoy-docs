# merchant-service — Technology Profile

> One-page technology reference for `merchant-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `merchant` (no partitioning — bounded by business count, not append-heavy)
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — merchant profile (TTL 5m)

## 5. External integrations

identity (Keycloak) · file (KYC documents)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile merchant up`
...
|---|---|---|---|---|---|---|---|
| Merchant profile | ✓ | ✓ | ✓ | read+reason | — | — | scrubbed |
| KYC documents (refs) | ✓ | ✓ | ✓ | read+reason | — | — | — |
...
|---|---|---|---|
| `POST` | `/admin/v1/merchants/{id}/suspend` | `platform.admin` | Suspend a merchant (immediate) |
| `POST` | `/admin/v1/merchants/{id}/kyc/approve` | `merchant.admin` | Approve a merchant's KYC submission |
| `POST` | `/admin/v1/merchants/{id}/kyc/reject` | `merchant.admin` | Reject a merchant's KYC submission (with reason) |
...
- This service's per-service admin role `merchant.admin`.
...
emit `audit.admin.merchant.v1` (per §10.2) for each touched
record.
