# search-service — Technology Profile

> One-page technology reference for `search-service`. The platform-wide
...
| | |
|---|---|
| **Profile** | Business core (read-heavy) |
| **Language** | Kotlin 2.2.x |
| **Framework** | Spring Boot 4.x + Spring Data OpenSearch 6 |
| **Build** | Gradle 9 (Kotlin DSL) |
| **Container** | `eclipse-temurin:25-jre-jammy` (multi-stage build, JRE-only final stage, non-root user) |

## 2. Key libraries

- Spring Data OpenSearch 6
- OpenSearch Java client (low-level)
- Spring Data JPA (Hibernate 7) — index metadata
...
Redis — query result cache (TTL 30s)
...
- **HPA signal**: CPU 60%, 3–15 replicas, p99 < 200ms
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile search up`
...
|---|---|---|---|
| `POST` | `/admin/v1/index/{vertical}/rebuild` | `search.admin` | Force-rebuild a vertical's index from upstream events |
| `POST` | `/admin/v1/index/{vertical}/reindex` | `search.admin` | Reindex a vertical with a new analyzer / mapping |
...
- This service's per-service admin role `search.admin`.
...
emit `audit.admin.search.v1` (per §10.2) for each touched
record.
