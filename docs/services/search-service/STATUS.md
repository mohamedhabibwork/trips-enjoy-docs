# search-service — Status Snapshot
...
> | Documentation completeness | filesystem scan (`docs/services/search-service/`) |
...
| Field | Value |
|---|---|
| Service name (kebab-case) | `search-service` |
| Bounded context | **Bounded Context**: *Search index coordination*. |
| Domain | Analytics |
| Tier (deployment) | 2 (position 19 of 21; `DEPLOYMENT_ORDER.md` §2) |
...
| DB schema | `search` (per-service) | `services/README.md` env-var table |
| Cache | Redis — query cache | `TECH.md` §4 |
| HPA signal | CPU 60%, 2–6, p99 < 200ms | `TECH.md` §8 |
| Replicas (default) | — | `TECH.md` §8 |
| p99 latency target | — | `TECH.md` §8 |
| Image | `registry.trips-enjoy.com/search-service:<sha>` | `README.md` §18 |
| Container port | 8080 | `TECH.md` §1 |
| Health endpoints | `/actuator/health/liveness`, `/actuator/health/readiness` | `TECH.md` §7 |
| `.env.example` | `apps/search-service/.env.example` ✅ | filesystem |
...
| `apps/search-service/k8s/` (flat kustomize overlays) | — |
| `apps/search-service/monitoring/` (ServiceMonitor + PrometheusRule) | — |
...
| Outbound REST APIs | 19 (INTEGRATION.md §2) |
| Produced events | 3 (INTEGRATION.md §3) |
| Consumed events | 10 (INTEGRATION.md §4) |
...
| AuthZ | RBAC; admin role `search-service.admin` per `TECH.md` §10 |
...
| Time-bounded alias | `platform-internal` realm `service-claims` scope mappers (per identity-service per-service claim contract); `search-service.scopes` / `search-service.level` / `search-service.tenant` claims available |
...
| Field | Value |
|---|---|
| Plan header | Domain: Analytics / Tier: 6 / DB Schema: `search` / Cache: Redis — query cache / HPA: CPU 60%, 2–6, p99 < 200ms |
...
| Plan task total | (count of `T-search-service`-prefixed rows in `PLAN.md`) |
...
- **Implementation memory** (graduates only): `uber-search-service-implementation-<date>.md` (project memory index)
