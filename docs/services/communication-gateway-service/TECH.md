# communication-gateway-service — Technology Profile

> One-page technology reference for `communication-gateway-service`. The platform-wide
...
- `go-redis/redis` v9 — delivery receipts, dedup
- `segmentio/kafka-go` — fan-out from `notification-service`
- `prometheus/client_golang` v1.20+ — metrics

## 3. Data layer

- **Database**: PostgreSQL 18, schema `comms_gateway` (daily RANGE partitioned on `window_started_at`)
- **DB extras**: daily `provider_health` partitions, pre-create 30 future days; monthly `sends` partitions, pre-create 12 future months
- **Migrations**: `golang-migrate` v4
- **ORM / DSL**: `pgx` v5 (raw SQL + `pgxpool`)

## 4. Cache

Redis — delivery receipts, dedup window

## 5. External integrations

FCM (Android push) · APNs (iOS push) · Twilio (SMS) · AWS SES (email)
...
- **HPA signal**: RPS, 3–50 replicas, p99 < 100ms
- **Pod resources**: requests/limits set per service (see `k8s/base/<service>/deployment.yaml`)

## 9. Local dev

- **Run**: `go run ./cmd/communication-gateway`
- **Test**: `go test ./...`
- **Compose profile**: `docker compose --profile communication-gateway up`
...
- **Key**: `audit.admin.communication_gateway.v1`
...
|---|---|---|---|---|---|---|---|
| Delivery state (Redis) | ✓ | ✓ | ✓ | — | — | — | — |
| Provider tokens | break-glass | — | — | — | — | — | — |
...
|---|---|---|---|
| `POST` | `/admin/v1/deliveries/{id}/retry` | `communication_gateway.admin` | Retry a failed delivery (push/SMS/email) |
| `POST` | `/admin/v1/providers/{name}/failover` | `platform.admin` | Fail over from one provider to another (e.g. Twilio → MessageBird) |
| `GET` | `/admin/v1/providers/health` | `platform.engineering` | Per-provider health and error-rate snapshot |
...
- **Run with admin port**: `ADMIN_PORT=8081 go run ./cmd/communication-gateway`
...
- This service's per-service admin role `communication_gateway.admin`.
...
emit `audit.admin.communication_gateway.v1` (per §10.2) for each touched
record.
...
**External vendor SDK.** FCM · APNs · Twilio · AWS SES (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §7 for the per-service index).

**Per-service OSS libraries.** This service pulls in the full pinned set listed in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §4 *Go OSS dependencies* (the `platform-spring-boot-starter` for Kotlin, the standard Go stack for Go, or the FastAPI + Pydantic + SQLAlchemy set for Python). The most service-specific entries are: `go-chi/chi v2` · `go-redis/redis v9` · `segmentio/kafka-go` · `prometheus/client_golang`.
