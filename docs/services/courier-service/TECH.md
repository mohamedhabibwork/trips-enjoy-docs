# courier-service — Technology Profile

> One-page technology reference for `courier-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `courier`
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — claim cache (TTL 5m)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile courier up`
...
|---|---|---|---|---|---|---|---|
| Courier profile | ✓ | ✓ | ✓ | read+reason | — | — | scrubbed |
| KYC documents (refs) | ✓ | ✓ | ✓ | read+reason | — | — | scrubbed |
...
|---|---|---|---|
| `POST` | `/admin/v1/couriers/{id}/suspend` | `platform.admin` | Suspend a courier (immediate; emits `courier.suspended.v1`) |
| `POST` | `/admin/v1/couriers/{id}/unsuspend` | `platform.admin` | Unsuspend a courier |
| `POST` | `/admin/v1/couriers/{id}/documents/verify` | `courier.admin` | Mark a courier document (ID, license) as verified |
...
- This service's per-service admin role `courier.admin`.
...
emit `audit.admin.courier.v1` (per 10.2) for each touched
record.
...
**External vendor SDK.** identity (Keycloak) (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) 7 for the per-service index).

**Per-service OSS libraries.** This service pulls in the full pinned set listed in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) 3 *Kotlin / Spring Boot OSS dependencies* (the `platform-spring-boot-starter` for Kotlin, the standard Go stack for Go, or the FastAPI + Pydantic + SQLAlchemy set for Python). The most service-specific entries are: Spring Data JPA · Spring Security 7 · MapStruct · Flyway.
