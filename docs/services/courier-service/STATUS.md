# courier-service — Status Snapshot
...
> | Documentation completeness | filesystem scan (`docs/services/courier-service/`) |
...
| Field | Value |
|---|---|
| Service name (kebab-case) | `courier-service` |
| Bounded context | **Courier profile + KYC.** In scope: courier profile, |
| Domain | Identity & Profile |
| Tier (deployment) | 1 (position 11 of 21; `DEPLOYMENT_ORDER.md` §2) |
...
| DB schema | `courier` (per-service) | `services/README.md` env-var table |
| Cache | Redis — claim cache (TTL 5m) | `TECH.md` §4 |
| HPA signal | CPU 60%, 2–5, p99 < 200ms | `TECH.md` §8 |
| Replicas (default) | — | `TECH.md` §8 |
| p99 latency target | — | `TECH.md` §8 |
| Image | `registry.trips-enjoy.com/courier-service:<sha>` | `README.md` §18 |
| Container port | 8080 | `TECH.md` §1 |
| Health endpoints | `/actuator/health/liveness`, `/actuator/health/readiness` | `TECH.md` §7 |
| `.env.example` | `apps/courier-service/.env.example` ✅ | filesystem |
...
|---|---|
| Status | ⏳ Stub |
| `apps/courier-service/` Dockerfile | — |
| `apps/courier-service/k8s/` (flat kustomize overlays) | — |
| `apps/courier-service/monitoring/` (ServiceMonitor + PrometheusRule) | — |
...
| Field | Count / Value |
|---|---|
| Inbound REST APIs | 20 (full contract in INTEGRATION.md §1) |
| Outbound REST APIs | 9 (INTEGRATION.md §2) |
| Produced events | 12 (INTEGRATION.md §3) |
| Consumed events | 12 (INTEGRATION.md §4) |
...
| AuthZ | RBAC; admin role `courier-service.admin` per `TECH.md` §10 |
...
| Time-bounded alias | `platform-internal` realm `service-claims` scope mappers (per identity-service per-service claim contract); `courier-service.scopes` / `courier-service.level` / `courier-service.tenant` claims available |
...
| Field | Value |
|---|---|
| Plan header | Domain: Identity & Profile / Tier: 1 / DB Schema: `courier` / Cache: Redis — claim cache (TTL 5m) / HPA: CPU 60%, 2–5, p99 < 200ms |
| Phase 7.0 (cross-cutting) block | — |
| Phase 7.5 (Make-a-Deal kernel) block | ✅ present |
| Phase 7.6 (Conductor workers) block | ✅ present |
| Phase 7.7 (in-app chat) block | ✅ present |
| Plan task total | (count of `T-courier-service`-prefixed rows in `PLAN.md`) |
...
- **Implementation memory** (graduates only): `uber-courier-service-implementation-<date>.md` (project memory index)
