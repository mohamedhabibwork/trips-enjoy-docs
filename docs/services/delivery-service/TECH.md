# delivery-service — Technology Profile

> One-page technology reference for `delivery-service`. The platform-wide
...
- Spring Statemachine 5 (delivery lifecycle)
- MapStruct
- Flyway 11

## 3. Data layer

- **Database**: PostgreSQL 18, schema `delivery`
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — delivery state mirror
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile delivery up`
...
|---|---|---|---|---|---|---|---|
| Delivery aggregate | ✓ | ✓ | ✓ | read+reason | — | — | ✓ |
| Courier assignment | ✓ | ✓ | ✓ | read+reason | — | — | ✓ |
...
|---|---|---|---|
| `POST` | `/admin/v1/deliveries/{id}/force-state` | `delivery.admin` | Force the delivery state machine to a target state (with warning event) |
| `POST` | `/admin/v1/deliveries/{id}/reassign` | `delivery.admin` | Reassign a delivery to a different courier (operational override) |
...
- This service's per-service admin role `delivery.admin`.
...
emit `audit.admin.delivery.v1` (per §10.2) for each touched
record.
