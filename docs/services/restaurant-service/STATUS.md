# restaurant-service — Status Snapshot
...
> | Documentation completeness | filesystem scan (`docs/services/restaurant-service/`) |
...
| Field | Value |
|---|---|
| Service name (kebab-case) | `restaurant-service` |
| Bounded context | - **In scope**: restaurant brand, type, cuisines, logo, ratings |
| Domain | Food Marketplace |
| Tier (deployment) | 1 (position 12 of 21; `DEPLOYMENT_ORDER.md` §2) |
...
| DB schema | `restaurant` (per-service) | `services/README.md` env-var table |
| Cache | Redis — restaurant profile | `TECH.md` §4 |
| HPA signal | CPU 60%, 2–4, p99 < 100ms | `TECH.md` §8 |
| Replicas (default) | — | `TECH.md` §8 |
| p99 latency target | — | `TECH.md` §8 |
| Image | `registry.trips-enjoy.com/restaurant-service:<sha>` | `README.md` §18 |
| Container port | 8080 | `TECH.md` §1 |
| Health endpoints | `/actuator/health/liveness`, `/actuator/health/readiness` | `TECH.md` §7 |
| `.env.example` | `apps/restaurant-service/.env.example` ✅ | filesystem |
...
| `apps/restaurant-service/k8s/` (flat kustomize overlays) | — |
| `apps/restaurant-service/monitoring/` (ServiceMonitor + PrometheusRule) | — |
...
| Field | Count / Value |
|---|---|
| Inbound REST APIs | 15 (full contract in INTEGRATION.md §1) |
| Outbound REST APIs | 6 (INTEGRATION.md §2) |
| Produced events | 8 (INTEGRATION.md §3) |
...
| AuthZ | RBAC; admin role `restaurant-service.admin` per `TECH.md` §10 |
...
| Time-bounded alias | `platform-internal` realm `service-claims` scope mappers (per identity-service per-service claim contract); `restaurant-service.scopes` / `restaurant-service.level` / `restaurant-service.tenant` claims available |
...
| Field | Value |
|---|---|
| Plan header | Domain: Food Marketplace / Tier: 3 / DB Schema: `restaurant` / Cache: Redis — restaurant profile / HPA: CPU 60%, 2–4, p99 < 100ms |
...
| Plan task total | (count of `T-restaurant-service`-prefixed rows in `PLAN.md`) |
...
- **Implementation memory** (graduates only): `uber-restaurant-service-implementation-<date>.md` (project memory index)
