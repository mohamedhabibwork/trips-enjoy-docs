...
- [ ] Create schema `restaurant`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `restaurant.outbox` and `restaurant.inbox` for reliable eventing
