# restaurant-service — Technology Profile

> One-page technology reference for `restaurant-service`. The platform-wide
...
- **Database**: PostgreSQL 19, schema `restaurant` (`pg_trgm` for fuzzy name search) (no partitioning — bounded by business count)
- **DB extras**: pg_trgm
...
Redis — restaurant profile (TTL 5m)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile restaurant up`
...
|---|---|---|---|---|---|---|---|
| Restaurant profile | ✓ | ✓ | ✓ | read+reason | — | — | read |
| Hours / settings | ✓ | ✓ | ✓ | read+reason | — | — | read |
...
|---|---|---|---|
| `POST` | `/admin/v1/restaurants/{id}/toggle-open` | `restaurant.admin` | Force-open / force-close a restaurant (operational override, e.g. health inspection) |
...
- This service's per-service admin role `restaurant.admin`.
...
emit `audit.admin.restaurant.v1` (per 10.2) for each touched
record.
