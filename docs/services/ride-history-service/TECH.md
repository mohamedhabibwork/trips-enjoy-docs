# ride-history-service — Technology Profile

> One-page technology reference for `ride-history-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `ride_history` (yearly RANGE partitioned on `trip_completed_at`, read-optimised)
- **DB extras**: yearly partitions; pre-create 2 future years; drop after 7 years
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — recent trips (TTL 5m)
...
- **HPA signal**: CPU 60%, 2–8 replicas, p99 < 500ms
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile ride-history up`
...
- `platform.support`
- `ride_history.admin`
- `ride_history.support`

### 10.2 Audit log

Every admin call on this service emits one event to:

- **Key**: `audit.admin.ride_history.v1`
...
|---|---|---|---|---|---|---|---|
| Trip history | ✓ | ✓ | ✓ | read+reason | — | — | scrubbed |
...
|---|---|---|---|
| `GET` | `/admin/v1/rides/{user_id}/history` | `platform.support` | Full ride history for a user (PII scrubbed unless `?reason=...`) |
| `POST` | `/admin/v1/rides/{trip_id}/pseudonymize` | `ride_history.admin` | Pseudonymize a trip record (GDPR / retention) |
...
- This service's per-service admin role `ride_history.admin`.
...
emit `audit.admin.ride_history.v1` (per §10.2) for each touched
record.
