...
This PLAN.md is the source of truth for **how** `promotion-service` is built. The 10-phase
...
- [ ] Create schema `promotion`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `promotion.outbox` and `promotion.inbox` for reliable eventing
...
- [ ] Sync dependencies: customer-service, configuration-service
...
|-------|-------|---------|--------------|
| `promotion.redeemed` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `promotion.created` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
