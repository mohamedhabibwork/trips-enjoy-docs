# promotion-service — Technology Profile

> One-page technology reference for `promotion-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `promotion` (monthly RANGE partitioned on `created_at`)
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — active promotion set (TTL 5m)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile promotion up`
...
|---|---|---|---|---|---|---|---|
| Promotion definitions | ✓ | ✓ | ✓ | — | — | — | read |
| Redemption history | ✓ | ✓ | read | — | — | — | read |
...
|---|---|---|---|
| `POST` | `/admin/v1/promotions/{id}/force-expire` | `promotion.admin` | Force-expire a promotion early (e.g. budget exhausted) |
| `POST` | `/admin/v1/promotions/{id}/force-disable` | `promotion.admin` | Disable a promotion without expiring it |
...
- This service's per-service admin role `promotion.admin`.
...
emit `audit.admin.promotion.v1` (per §10.2) for each touched
record.
