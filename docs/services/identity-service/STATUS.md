# identity-service — Status Snapshot
...
> | Documentation completeness | filesystem scan (`docs/services/identity-service/`) |
...
| Field | Value |
|---|---|
| Service name (kebab-case) | `identity-service` |
| Bounded context | **Identity / Keycloak adapter.** In scope: Keycloak federation, |
| Domain | Platform Foundation |
| Tier (deployment) | 0 (position 2 of 21; `DEPLOYMENT_ORDER.md` §2) |
...
| DB schema | `identity` (per-service) | `services/README.md` env-var table |
| Cache | Redis — session+token | `TECH.md` §4 |
| HPA signal | CPU 70%, 3–8, p99 < 80ms | `TECH.md` §8 |
| Replicas (default) | — | `TECH.md` §8 |
| p99 latency target | — | `TECH.md` §8 |
| Image | `registry.trips-enjoy.com/identity-service:<sha>` | `README.md` §18 |
| Container port | 8080 | `TECH.md` §1 |
| Health endpoints | `/actuator/health/liveness`, `/actuator/health/readiness` | `TECH.md` §7 |
| `.env.example` | `apps/identity-service/.env.example` ✅ | filesystem |
...
|---|---|
| Status | ✅ Graduated |
| `apps/identity-service/` Dockerfile | ✅ present |
| `apps/identity-service/k8s/` (flat kustomize overlays) | ✅ present |
| `apps/identity-service/monitoring/` (ServiceMonitor + PrometheusRule) | ✅ present |
| Local test suite | (see §8.2 row above) |
| Implementation memory | see `uber-identity-service-implementation-*.md` in project memory index |
...
| Outbound REST APIs | 10 (INTEGRATION.md §2) |
| Produced events | 9 (INTEGRATION.md §3) |
| Consumed events | 6 (INTEGRATION.md §4) |
...
| AuthZ | RBAC; admin role `identity-service.admin` per `TECH.md` §10 |
...
| Time-bounded alias | `platform-internal` realm `service-claims` scope mappers (per identity-service per-service claim contract); `identity-service.scopes` / `identity-service.level` / `identity-service.tenant` claims available |
...
| Field | Value |
|---|---|
| Plan header | Domain: Platform Foundation / Tier: 1 / DB Schema: `identity` / Cache: Redis — session+token / HPA: CPU 70%, 3–8, p99 < 80ms |
...
| Plan task total | (count of `T-identity-service`-prefixed rows in `PLAN.md`) |
...
- **Implementation memory** (graduates only): `uber-identity-service-implementation-<date>.md` (project memory index)
