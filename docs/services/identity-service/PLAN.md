...
- [ ] Create schema `identity`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `identity.outbox` and `identity.inbox` for reliable eventing
