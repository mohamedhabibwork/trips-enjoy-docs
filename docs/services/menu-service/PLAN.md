# menu-service — Implementation Plan
...
**Criticality:** T2 (99.9%)
**DB Schema:** `menu`
**Cache:** Redis — menu snapshot
**HPA:** CPU 60%, 2–6, p99 < 100ms

---

## Purpose

**Phase 4 — Food Marketplace.** Begin once Phase 4 menu + cart are live.

This PLAN.md is the source of truth for **how** `menu-service` is built. The 10-phase
...
- [ ] Create schema `menu`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `menu.outbox` and `menu.inbox` for reliable eventing
...
- [ ] Sync dependencies: restaurant-service, tax-service, inventory-service
...
| `inventory-service` | per `INTEGRATION.md` | sync dependency | Yes |

### Events Published
| Event | Topic | Trigger | Key Consumers |
|-------|-------|---------|--------------|
| `menu.created` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `menu.updated` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `menu.item.unavailable` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `menu.item.price.changed` | derived from name | see INTEGRATION.md | see INTEGRATION.md |

### Events Consumed
| Event | Producer | Handler |
|-------|----------|---------|
| `restaurant.created` | see INTEGRATION.md | see INTEGRATION.md |
| `inventory.item.86d` | see INTEGRATION.md | see INTEGRATION.md |
