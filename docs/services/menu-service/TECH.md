# menu-service — Technology Profile

> One-page technology reference for `menu-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `menu` (`pg_trgm` extension for fuzzy name search) (no partitioning — millions globally, access by restaurant_id not time-windowed)
- **DB extras**: pg_trgm
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA) + `pg_trgm` for search

## 4. Cache

Redis — menu tree (TTL 1h, push-invalidate on menu update)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile menu up`
...
|---|---|---|---|---|---|---|---|
| Menu items / categories | ✓ | ✓ | ✓ | read+reason | — | — | read |
| Photos (file refs) | ✓ | ✓ | ✓ | — | — | — | — |
...
|---|---|---|---|
| `POST` | `/admin/v1/menus/{restaurant_id}/86-item` | `menu.admin` | 86 a specific item (mark unavailable) |
| `POST` | `/admin/v1/menus/{restaurant_id}/un86-item` | `menu.admin` | Un-86 an item |
...
- This service's per-service admin role `menu.admin`.
...
emit `audit.admin.menu.v1` (per §10.2) for each touched
record.
...
**External vendor SDK.** file (photos) (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §7 for the per-service index).

**Per-service OSS libraries.** This service pulls in the full pinned set listed in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §3 *Kotlin / Spring Boot OSS dependencies* (the `platform-spring-boot-starter` for Kotlin, the standard Go stack for Go, or the FastAPI + Pydantic + SQLAlchemy set for Python). The most service-specific entries are: Spring Data JPA · Spring Cache (Caffeine + Redis) · Flyway.
