# driver-availability-service — Technology Profile

> One-page technology reference for `driver-availability-service`. The platform-wide
...
- `go-redis/redis` v9 — driver online/availability state
- `prometheus/client_golang` v1.20+ — metrics

## 3. Data layer

- **Database**: PostgreSQL 18, schema `driver_availability` (daily RANGE partitioned on `occurred_at`)
...
Redis — online/availability state mirror
...
- **HPA signal**: RPS, 2–20 replicas, p99 < 20ms
- **Pod resources**: requests/limits set per service (see `k8s/base/<service>/deployment.yaml`)

## 9. Local dev

- **Run**: `go run ./cmd/driver-availability`
- **Test**: `go test ./...`
- **Compose profile**: `docker compose --profile driver-availability up`
...
- **Key**: `audit.admin.driver_availability.v1`
...
|---|---|---|---|
| `POST` | `/admin/v1/drivers/{id}/force-offline` | `platform.admin` | Force a driver offline (e.g. in response to a safety report) |
| `POST` | `/admin/v1/availability/resync` | `driver_availability.admin` | Re-sync availability state from DB → Redis (operational recovery) |
...
- **Run with admin port**: `ADMIN_PORT=8081 go run ./cmd/driver-availability`
...
- This service's per-service admin role `driver_availability.admin`.
...
emit `audit.admin.driver_availability.v1` (per §10.2) for each touched
record.
