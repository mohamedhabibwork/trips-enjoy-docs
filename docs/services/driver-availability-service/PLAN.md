# driver-availability-service — Implementation Plan

**Domain:** Ride-Hailing
**Tier:** 3
**Technology:** Go
**Criticality:** T1 (99.95%)
**DB Schema:** `driver_availability`
**Cache:** Redis — bitmap + geoset
**HPA:** CPU 70%, 3–8, p99 < 50ms
...
This PLAN.md is the source of truth for **how** `driver-availability-service` is built. The 10-phase
...
- [ ] Create schema `driver_availability`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `driver_availability.outbox` and `driver_availability.inbox` for reliable eventing
...
- [ ] Sync dependencies: driver-service
...
|-------|-------|---------|--------------|
| `driver.availability.online` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `driver.availability.offline` | derived from name | see INTEGRATION.md | see INTEGRATION.md |

### Events Consumed
| Event | Producer | Handler |
|-------|----------|---------|
| `driver.approved` | see INTEGRATION.md | see INTEGRATION.md |
| `driver.suspended` | see INTEGRATION.md | see INTEGRATION.md |
