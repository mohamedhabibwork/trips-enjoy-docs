# cart-service — Technology Profile

> One-page technology reference for `cart-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `cart` (no partitioning — short-lived, aggressively pruned)
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — active cart session (TTL 30m)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile cart up`
...
|---|---|---|---|---|---|---|---|
| Cart aggregates | ✓ | ✓ | ✓ | read+reason | — | — | ✓ |
| Customer identifiers | ✓ | ✓ | scrubbed | scrubbed+reason | — | — | scrubbed |
...
|---|---|---|---|
| `GET` | `/admin/v1/carts/{user_id}` | `platform.support` | List all open carts for a user (incl. abandoned) |
| `POST` | `/admin/v1/carts/{cart_id}/abandon` | `cart.admin` | Force-abandon a stuck cart (e.g. abandoned for >24h) |
...
- This service's per-service admin role `cart.admin`.
...
emit `audit.admin.cart.v1` (per §10.2) for each touched
record.
