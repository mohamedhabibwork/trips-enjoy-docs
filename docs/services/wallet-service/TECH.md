# wallet-service — Technology Profile

> One-page technology reference for `wallet-service`. The platform-wide
...
- Spring Statemachine 5 (hold / release / capture)
- MapStruct

## 3. Data layer

- **Database**: PostgreSQL 18, schema `wallet` (monthly RANGE partitioned on `occurred_at`)
...
Redis — balance (TTL 30s, push-invalidate on every wallet transaction)
...
- **HPA signal**: CPU 60% + RPS, 3–20 replicas, p99 < 200ms
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile wallet up`
...
|---|---|---|---|
| `POST` | `/admin/v1/wallets/{id}/adjust` | `wallet.admin` | Adjust a wallet balance (with double-entry pairing in `ledger-service`; with audit) |
| `GET` | `/admin/v1/wallets/{id}/history` | `wallet.admin` | Full transaction history for a wallet (incl. holds, releases, captures) |
| `POST` | `/admin/v1/wallets/{id}/force-release` | `wallet.admin` | Force-release all holds on a wallet (e.g. stuck reservation) |
...
- This service's per-service admin role `wallet.admin`.
...
emit `audit.admin.wallet.v1` (per §10.2) for each touched
record.
