# configuration-service — Technology Profile

> One-page technology reference for `configuration-service`. The platform-wide
...
- Spring Cache (Caffeine + Redis long-poll)
- Flyway 11

## 3. Data layer

- **Database**: PostgreSQL 18, schema `configuration` (monthly RANGE partitioned on `created_at`)
...
Redis — long-poll / push-invalidate pattern
...
- **HPA signal**: CPU 60%, 2–5 replicas, p99 < 50ms
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile configuration up`
...
- **Key**: `audit.admin.configuration.v1`
...
|---|---|---|---|---|---|---|---|
| Business rules & values | ✓ | ✓ | read | — | — | — | read |
| Config history | ✓ | ✓ | read | — | — | — | read |
...
|---|---|---|---|
| `POST` | `/admin/v1/config/{key}/rollback` | `configuration.admin` | Roll a config key back to its previous version (with audit) |
| `GET` | `/admin/v1/config/history` | `configuration.admin` | Full version history for a config key (with author + reason) |
| `POST` | `/admin/v1/config/bulk-publish` | `configuration.admin` | Atomically publish a batch of config changes to subscribers |
...
- This service's per-service admin role `configuration.admin`.
...
emit `audit.admin.configuration.v1` (per 10.2) for each touched
record.
