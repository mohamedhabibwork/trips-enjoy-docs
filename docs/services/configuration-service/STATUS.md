# configuration-service — Status Snapshot
...
> | Documentation completeness | filesystem scan (`docs/services/configuration-service/`) |
...
| Field | Value |
|---|---|
| Service name (kebab-case) | `configuration-service` |
| Bounded context | Externalized configuration. The service is the |
| Domain | Platform Foundation |
| Tier (deployment) | 0 (position 1 of 21; `DEPLOYMENT_ORDER.md` §2) |
...
| DB schema | `configuration` (per-service) | `services/README.md` env-var table |
| Cache | Redis — long-poll / push-invalidate | `TECH.md` §4 |
| HPA signal | CPU 60%, 2–5, p99 < 50ms | `TECH.md` §8 |
| Replicas (default) | — | `TECH.md` §8 |
| p99 latency target | — | `TECH.md` §8 |
| Image | `registry.trips-enjoy.com/configuration-service:<sha>` | `README.md` §18 |
| Container port | 8080 | `TECH.md` §1 |
| Health endpoints | `/actuator/health/liveness`, `/actuator/health/readiness` | `TECH.md` §7 |
| `.env.example` | `apps/configuration-service/.env.example` ✅ | filesystem |
...
|---|---|
| Status | ✅ Graduated |
| `apps/configuration-service/` Dockerfile | ✅ present |
| `apps/configuration-service/k8s/` (flat kustomize overlays) | ✅ present |
| `apps/configuration-service/monitoring/` (ServiceMonitor + PrometheusRule) | ✅ present |
| Local test suite | (see §8.2 row above) |
| Implementation memory | see `uber-configuration-service-implementation-*.md` in project memory index |
...
| Field | Count / Value |
|---|---|
| Inbound REST APIs | 9 (full contract in INTEGRATION.md §1) |
...
| AuthZ | RBAC; admin role `configuration-service.admin` per `TECH.md` §10 |
...
| Time-bounded alias | `platform-internal` realm `service-claims` scope mappers (per identity-service per-service claim contract); `configuration-service.scopes` / `configuration-service.level` / `configuration-service.tenant` claims available |
...
| Field | Value |
|---|---|
| Plan header | Domain: Platform Foundation / Tier: 1 / DB Schema: `configuration` / Cache: Redis — long-poll / push-invalidate / HPA: CPU 60%, 2–5, p99 < 50ms |
...
| Plan task total | (count of `T-configuration-service`-prefixed rows in `PLAN.md`) |
...
- **Implementation memory** (graduates only): `uber-configuration-service-implementation-<date>.md` (project memory index)
