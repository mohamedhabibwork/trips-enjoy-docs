# courier-dispatch-service — Technology Profile

> One-page technology reference for `courier-dispatch-service`. The platform-wide
...
- NumPy 2 (vectorised scoring)
- `aiokafka` (event consumer)
- `asyncpg` (PostgreSQL 18 async driver)

## 3. Data layer

- **Database**: PostgreSQL 18, schema `courier_dispatch` (monthly RANGE partitioned on `assigned_at`)
- **Migrations**: Alembic 1.13+
- **ORM / DSL**: `asyncpg` (raw SQL) — kept light for hot path
- **DB extras**: monthly partitions; pre-create 12 future months

## 4. Cache

Redis — match attempts (TTL 5m)
...
- **HPA signal**: RPS, 2–8 replicas, p99 < 200ms
...
- **Test**: `pytest`
- **Compose profile**: `docker compose --profile courier-dispatch up`
...
- **Key**: `audit.admin.courier_dispatch.v1`
...
|---|---|---|---|
| `GET` | `/admin/v1/matches/recent` | `platform.ops` | List recent match attempts with scoring breakdown |
| `POST` | `/admin/v1/matches/{request_id}/replay` | `courier_dispatch.admin` | Replay a match attempt (e.g. after a model update) |
...
- This service's per-service admin role `courier_dispatch.admin`.
...
emit `audit.admin.courier_dispatch.v1` (per §10.2) for each touched
record.
