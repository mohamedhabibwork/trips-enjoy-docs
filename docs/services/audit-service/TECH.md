# audit-service — Technology Profile

> One-page technology reference for `audit-service`. The platform-wide
...
- Spring Kafka 4 (consumer + EOS)
- Spring Data JPA (Hibernate 7) — append-only writer
- Flyway 11

## 3. Data layer

- **Database**: PostgreSQL 19, schema `audit` (append-only, monthly partitioning)
- **DB extras**: monthly partitions
...
- **HPA signal**: Kafka consumer lag, 2–8 replicas, 20k evt/s
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile audit up`
...
- **Key**: `audit.admin.audit.v1` (self-audit; rare)
...
|---|---|---|---|
| `GET` | `/admin/v1/audit/search` | `platform.admin` | Full-text search across the audit log (actor, endpoint, time range) |
| `POST` | `/admin/v1/audit/export` | `platform.admin` | Export the audit log to S3 for compliance (CSV / Parquet) |
| `POST` | `/admin/v1/audit/reindex` | `audit.admin` | Rebuild the audit search index from the append-only log |
...
- This service's per-service admin role `audit.admin`.
...
emit `audit.admin.audit.v1` (per 10.2) for each touched
record.
...
**External vendor SDK.** S3 (cold archive) (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) 7 for the per-service index).

**Per-service OSS libraries.** This service pulls in the full pinned set listed in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) 3 *Kotlin / Spring Boot OSS dependencies* (the `platform-spring-boot-starter` for Kotlin, the standard Go stack for Go, or the FastAPI + Pydantic + SQLAlchemy set for Python). The most service-specific entries are: Spring Kafka 4 · Spring Data JPA · Flyway.
