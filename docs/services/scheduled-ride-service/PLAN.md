# scheduled-ride-service — Implementation Plan

**Domain:** Ride-Hailing
**Tier:** 4
**Technology:** Kotlin/Spring
**Criticality:** T2 (99.9%)
**DB Schema:** `scheduled_ride`
**Cache:** Redis — cron registry
...
This PLAN.md is the source of truth for **how** `scheduled-ride-service` is built. The 10-phase
...
- [ ] Create schema `scheduled_ride`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `scheduled_ride.outbox` and `scheduled_ride.inbox` for reliable eventing
...
- [ ] Sync dependencies: ride-request-service
...
|-------|-------|---------|--------------|
| `scheduled_ride.due` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
