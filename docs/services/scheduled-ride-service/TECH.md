# scheduled-ride-service — Technology Profile

> One-page technology reference for `scheduled-ride-service`. The platform-wide
...
- Spring Scheduling (Quartz for distributed scheduling)
- MapStruct
- Flyway 11

## 3. Data layer

- **Database**: PostgreSQL 18, schema `scheduled_ride` (monthly RANGE partitioned on `scheduled_for` / `attempted_at`)
...
Redis — soon-to-fire sorted set (ZADD with score = fire timestamp)

## 5. External integrations

ride-request (fires scheduled bookings)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile scheduled-ride up`
...
- `scheduled_ride.support`

### 10.2 Audit log

Every admin call on this service emits one event to:

- **Key**: `audit.admin.scheduled_ride.v1`
...
|---|---|---|---|---|---|---|---|
| Scheduled bookings | ✓ | ✓ | ✓ | read+reason | — | — | scrubbed |
...
|---|---|---|---|
| `POST` | `/admin/v1/scheduled-rides/{id}/fire` | `scheduled_ride.admin` | Fire a scheduled ride now (e.g. customer wants early pickup) |
| `POST` | `/admin/v1/scheduled-rides/{id}/reschedule` | `scheduled_ride.admin` | Reschedule a scheduled ride (with audit) |
...
- This service's per-service admin role `scheduled_ride.admin`.
...
emit `audit.admin.scheduled_ride.v1` (per §10.2) for each touched
record.
