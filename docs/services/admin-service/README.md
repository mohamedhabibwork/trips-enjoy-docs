...
| **BFF admin endpoints** (graduated services) | `/v1/admin/{service}/...` | per-service `<service>.admin` (writes) / `platform.engineering` (reads) | end-to-end management wrappers for the 9 graduated services (`configuration-service`, `identity-service`, `audit-service`, `api-gateway`, `file-service`, `geolocation-service`, `notification-service`, `ledger-service`, `reporting-service`). 60+ endpoints enumerated in [`INTEGRATION.md` §1.22](./INTEGRATION.md#122-bff-admin-endpoints-for-graduated-services-end-to-end-management). Each call emits `admin.action.performed.v1`, enforces `X-Audit-Reason` + `Idempotency-Key` + `X-Signature`, applies break-glass per the table in §1.22.11, and forwards to the target service's own admin API. |
| Cross-service aggregations | `/v1/admin/services/{service}/overview`, `/v1/admin/services/{service}/config-keys`, `/v1/admin/audit/cross-service`, `/v1/admin/security/revoke-everywhere/{identity_id}` | per-endpoint (see [`INTEGRATION.md` §1.22.10](./INTEGRATION.md#12210-cross-service-bff-aggregations)) | multi-service fan-out for the operator UI; `/security/revoke-everywhere` is the panic button (platform.super_admin + break-glass + signature + MFA + super-admin IP allowlist) |
...
- **Depends on**: [`audit-service`](../audit-service/README.md), [`chat-service`](../chat-service/README.md) *(Phase 7.7 — consumer of `chat.message.reported.v1`; opens support ticket)*, [`configuration-service`](../configuration-service/README.md), [`identity-service`](../identity-service/README.md), [`notification-service`](../notification-service/README.md), [`payment-service`](../payment-service/README.md)
- **Depended on by**: [`api-gateway`](../api-gateway/README.md), [`audit-service`](../audit-service/README.md), [`chat-service`](../chat-service/README.md) *(Phase 7.7 — admin moderation reads /admin/v1/chat/threads/{id} with chat.admin role)*, [`courier-service`](../courier-service/README.md), [`customer-service`](../customer-service/README.md), [`driver-service`](../driver-service/README.md), [`file-service`](../file-service/README.md), [`food-order-service`](../food-order-service/README.md), [`fraud-risk-service`](../fraud-risk-service/README.md), [`notification-service`](../notification-service/README.md), [`payment-service`](../payment-service/README.md), [`pricing-service`](../pricing-service/README.md), [`reporting-service`](../reporting-service/README.md), [`restaurant-service`](../restaurant-service/README.md), [`trip-service`](../trip-service/README.md)
