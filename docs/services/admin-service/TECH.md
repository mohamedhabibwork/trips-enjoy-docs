...
- **Database**: PostgreSQL 18, schema `admin` (BFF — minimal own state)
...
| `GET` | `/admin/v1/services` | `platform.admin` | Service catalog: all 58 services with their accepted admin scopes (§10.1) and `SUPER_ADMIN` preset membership (§10.7). Used by the operator UI to preview what a super-admin grant touches |
