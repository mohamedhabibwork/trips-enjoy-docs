...
> | Documentation completeness | filesystem scan (`docs/services/admin-service/`) |
...
| Field | Value |
|---|---|
| Service name (kebab-case) | `admin-service` |
| Bounded context | Admin operations. In scope: |
| Domain | Platform & Operations |
| Tier (deployment) | 1 (position 13 of 21; `DEPLOYMENT_ORDER.md` §2) |
...
| DB schema | `admin` (per-service) | `services/README.md` env-var table |
| Cache | — | `TECH.md` §4 |
| HPA signal | CPU 60%, 2–5, p99 < 500ms | `TECH.md` §8 |
| Replicas (default) | — | `TECH.md` §8 |
| p99 latency target | — | `TECH.md` §8 |
| Image | `registry.trips-enjoy.com/admin-service:<sha>` | `README.md` §18 |
| Container port | 8080 | `TECH.md` §1 |
| Health endpoints | `/actuator/health/liveness`, `/actuator/health/readiness` | `TECH.md` §7 |
| `.env.example` | `apps/admin-service/.env.example` ✅ | filesystem |
...
| `apps/admin-service/k8s/` (flat kustomize overlays) | — |
| `apps/admin-service/monitoring/` (ServiceMonitor + PrometheusRule) | — |
...
| Outbound REST APIs | 5 (INTEGRATION.md §2) |
...
| AuthZ | RBAC; admin role `admin-service.admin` per `TECH.md` §10 |
...
| Time-bounded alias | `platform-internal` realm `service-claims` scope mappers (per identity-service per-service claim contract); `admin-service.scopes` / `admin-service.level` / `admin-service.tenant` claims available |
...
| Field | Value |
|---|---|
| Plan header | Domain: Platform & Operations / Tier: 1 / DB Schema: `admin` / Cache: — / HPA: CPU 60%, 2–5, p99 < 500ms |
...
| Plan task total | (count of `T-admin-service`-prefixed rows in `PLAN.md`) |
...
- **Implementation memory** (graduates only): `uber-admin-service-implementation-<date>.md` (project memory index)
