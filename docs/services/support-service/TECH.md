# support-service — Technology Profile

> One-page technology reference for `support-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `support`
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — agent presence / routing

## 5. External integrations

file (ticket attachments) · communication-gateway (agent-customer chat)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile support up`
...
|---|---|---|---|---|---|---|---|
| Tickets | ✓ | ✓ | read | ✓ | — | — | scrubbed |
| Customer PII | ✓ | ✓ | scrubbed | scrubbed+reason | — | — | scrubbed |
...
|---|---|---|---|
| `GET` | `/admin/v1/tickets/{id}/customer-context` | `platform.support` | Full customer context (PII) for a ticket — requires `?reason=...` |
| `POST` | `/admin/v1/tickets/{id}/reassign` | `support.admin` | Reassign a ticket to a different agent or queue |
...
- This service's per-service admin role `support.admin`.
...
emit `audit.admin.support.v1` (per §10.2) for each touched
record.
...
**External vendor SDK.** file · communication-gateway (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §7 for the per-service index).
