# support-service — Implementation Plan
...
This PLAN.md is the source of truth for **how** `support-service` is built. The 10-phase
...
- [ ] Create schema `support`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `support.outbox` and `support.inbox` for reliable eventing
...
- [ ] Sync dependencies: customer-service, driver-service, courier-service, payment-service
...
|-------|-------|---------|--------------|
| `support.ticket.opened` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `support.ticket.resolved` | derived from name | see INTEGRATION.md | see INTEGRATION.md |

### Events Consumed
| Event | Producer | Handler |
|-------|----------|---------|
| `payment.disputed` | see INTEGRATION.md | see INTEGRATION.md |
