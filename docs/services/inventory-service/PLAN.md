...
This PLAN.md is the source of truth for **how** `inventory-service` is built. The 10-phase
...
- [ ] Create schema `inventory`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `inventory.outbox` and `inventory.inbox` for reliable eventing
...
- [ ] Sync dependencies: menu-service
...
|-------|-------|---------|--------------|
| `inventory.item.out_of_stock` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `inventory.item.restocked` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `inventory.item.86d` | derived from name | see INTEGRATION.md | see INTEGRATION.md |

### Events Consumed
| Event | Producer | Handler |
|-------|----------|---------|
| `menu.item.unavailable` | see INTEGRATION.md | see INTEGRATION.md |
