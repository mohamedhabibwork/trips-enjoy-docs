# inventory-service — Technology Profile

> One-page technology reference for `inventory-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `inventory` (no partitioning — stock_movements bounded by job-driven pruning)
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — stock counts (TTL 1m, push-invalidate on order events)
...
- **HPA signal**: CPU 60%, 2–8 replicas, p99 < 200ms
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile inventory up`
...
|---|---|---|---|
| `POST` | `/admin/v1/inventory/{item_id}/adjust` | `inventory.admin` | Adjust stock count for an item (with audit + reason code) |
| `POST` | `/admin/v1/inventory/{item_id}/force-available` | `inventory.admin` | Override the 86'd state on a menu item (force-available) |
...
- This service's per-service admin role `inventory.admin`.
...
emit `audit.admin.inventory.v1` (per §10.2) for each touched
record.
...
**External vendor SDK.** menu (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §7 for the per-service index).
