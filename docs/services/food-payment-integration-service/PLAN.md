# food-payment-integration-service — Implementation Plan

**Domain:** Food Delivery
**Tier:** 5
**Technology:** Kotlin/Spring
**Criticality:** T1 (99.95%)
**DB Schema:** `food_payment_integration`
...
This PLAN.md is the source of truth for **how** `food-payment-integration-service` is built. The 10-phase
...
- [ ] Create schema `food_payment_integration`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `food_payment_integration.outbox` and `food_payment_integration.inbox` for reliable eventing
...
- [ ] Sync dependencies: payment-service, wallet-service, ledger-service, restaurant-settlement-service, courier-earnings-service
...
| `restaurant-settlement-service` | per `INTEGRATION.md` | sync dependency | Yes |
| `courier-earnings-service` | per `INTEGRATION.md` | sync dependency | Yes |

### Events Published
| Event | Topic | Trigger | Key Consumers |
|-------|-------|---------|--------------|
| `food.payment.completed` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `merchant.settlement.created` | derived from name | see INTEGRATION.md | see INTEGRATION.md |

### Events Consumed
| Event | Producer | Handler |
|-------|----------|---------|
| `delivery.completed` | see INTEGRATION.md | see INTEGRATION.md |
