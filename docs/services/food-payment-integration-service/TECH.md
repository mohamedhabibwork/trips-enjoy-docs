# food-payment-integration-service — Technology Profile

> One-page technology reference for `food-payment-integration-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `food_payment_integration`
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile food-payment-integration up`
...
- `food_payment_integration.finance`

### 10.2 Audit log

Every admin call on this service emits one event to:

- **Key**: `audit.admin.food_payment_integration.v1`
...
|---|---|---|---|
| `POST` | `/admin/v1/sagas/{saga_id}/replay` | `food_payment_integration.admin` | Replay a stuck saga step (e.g. payment → ledger) |
| `GET` | `/admin/v1/sagas/{saga_id}/trace` | `food_payment_integration.admin` | Full saga trace with per-step timing and downstream state |
| `POST` | `/admin/v1/sagas/{saga_id}/force-state` | `food_payment_integration.admin` | Force the saga state machine to a target state (with warning event) |
...
- This service's per-service admin role `food_payment_integration.admin`.
...
emit `audit.admin.food_payment_integration.v1` (per §10.2) for each touched
record.
...
**External vendor SDK.** payment · ledger (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §7 for the per-service index).
