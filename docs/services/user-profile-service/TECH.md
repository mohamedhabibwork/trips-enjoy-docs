# user-profile-service — Technology Profile

> One-page technology reference for `user-profile-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `user_profile` (no partitioning — bounded by user count, identity-keyed)
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — profile (TTL 5m)

## 5. External integrations

identity (claims) · file (avatars)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile user-profile up`
...
- **Key**: `audit.admin.user_profile.v1`
...
|---|---|---|---|---|---|---|---|
| User profile | ✓ | ✓ | ✓ | read+reason | — | — | scrubbed |
| PII (name, email, phone, locale) | ✓ | ✓ | scrubbed | scrubbed+reason | — | — | scrubbed |
...
|---|---|---|---|
| `GET` | `/admin/v1/profiles/{user_id}` | `platform.support` | Full user profile (PII included, with `?reason=...`) |
| `POST` | `/admin/v1/profiles/{id}/pseudonymize` | `user_profile.admin` | Pseudonymize a user profile (GDPR / retention) |
...
- This service's per-service admin role `user_profile.admin`.
...
emit `audit.admin.user_profile.v1` (per §10.2) for each touched
record.
...
**External vendor SDK.** identity · file (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §7 for the per-service index).
