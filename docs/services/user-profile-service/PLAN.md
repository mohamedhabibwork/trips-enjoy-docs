# user-profile-service — Implementation Plan
...
**Cache:** Redis — per-user profile
**HPA:** CPU 60%, 2–4, p99 < 80ms
...
This PLAN.md is the source of truth for **how** `user-profile-service` is built. The 10-phase
...
- [ ] Create schema `user_profile`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `user_profile.outbox` and `user_profile.inbox` for reliable eventing
...
|-------|-------|---------|--------------|
| `user.profile.updated` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `user.preference.updated` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
