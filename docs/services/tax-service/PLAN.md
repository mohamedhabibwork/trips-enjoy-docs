...
**Criticality:** T2 (99.9%)
**DB Schema:** `tax`
**Cache:** Redis — jurisdiction rates
...
This PLAN.md is the source of truth for **how** `tax-service` is built. The 10-phase
...
- [ ] Create schema `tax`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `tax.outbox` and `tax.inbox` for reliable eventing
...
|-------|-------|---------|--------------|
| `tax.calculated` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
