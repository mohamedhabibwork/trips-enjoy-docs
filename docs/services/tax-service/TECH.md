# tax-service — Technology Profile

> One-page technology reference for `tax-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `tax` (monthly RANGE partitioned on `created_at`)
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Spring Data R2DBC (non-blocking)

## 4. Cache

Redis — rule snapshot (TTL 24h, push-invalidate on configuration change)

## 5. External integrations

configuration (tax rule source)
...
- **HPA signal**: CPU 60%, 2–8 replicas, p99 < 100ms
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile tax up`
...
|---|---|---|---|
| `POST` | `/admin/v1/tax/recalculate/{invoice_id}` | `tax.admin` | Recompute tax for a specific invoice (e.g. after a rule change) |
| `POST` | `/admin/v1/tax/rules/reload` | `tax.admin` | Force-reload the rule snapshot from `configuration-service` (push-invalidate bypass) |
...
- This service's per-service admin role `tax.admin`.
...
emit `audit.admin.tax.v1` (per §10.2) for each touched
record.
