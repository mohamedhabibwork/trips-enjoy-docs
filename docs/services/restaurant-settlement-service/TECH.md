# restaurant-settlement-service — Technology Profile

> One-page technology reference for `restaurant-settlement-service`. The platform-wide
...
- Spring Batch (settlement runs)
- Spring Data JPA (Hibernate 7) — read models
- MapStruct

## 3. Data layer

- **Database**: PostgreSQL 18, schema `restaurant_settlement` (monthly RANGE partitioned on `created_at`)
...
- **HPA signal**: CPU 60%, 2–5 replicas, p99 < 5s (batch)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile restaurant-settlement up`
...
- `restaurant_settlement.finance`

### 10.2 Audit log

Every admin call on this service emits one event to:

- **Key**: `audit.admin.restaurant_settlement.v1`
...
|---|---|---|---|---|---|---|---|
| Settlement runs (money) | ✓ | ✓ | — | — | ✓ | — | read |
| Currency conversions | ✓ | ✓ | — | — | ✓ | — | read |
...
|---|---|---|---|
| `POST` | `/admin/v1/settlements/{id}/recalculate` | `restaurant_settlement.admin` | Recompute a settlement run after a dispute / fee change |
| `POST` | `/admin/v1/settlements/{id}/force-publish` | `restaurant_settlement.admin` | Force-publish a settlement to the restaurant (bypass hold window) |
...
- This service's per-service admin role `restaurant_settlement.admin`.
...
emit `audit.admin.restaurant_settlement.v1` (per §10.2) for each touched
record.
...
**External vendor SDK.** ledger (read) (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §7 for the per-service index).

**Per-service OSS libraries.** This service pulls in the full pinned set listed in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §3 *Kotlin / Spring Boot OSS dependencies* (the `platform-spring-boot-starter` for Kotlin, the standard Go stack for Go, or the FastAPI + Pydantic + SQLAlchemy set for Python). The most service-specific entries are: jOOQ 3.20 · Spring Batch · Spring Data JPA (read) · MapStruct.
