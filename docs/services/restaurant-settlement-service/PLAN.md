# restaurant-settlement-service — Implementation Plan

**Domain:** Food Delivery
**Tier:** 5
**Technology:** Kotlin/Spring
**Criticality:** T1 (99.95%)
**DB Schema:** `restaurant_settlement`
**Cache:** Redis — payout run
**HPA:** CPU 60%, 2–4, p99 < 300ms
...
This PLAN.md is the source of truth for **how** `restaurant-settlement-service` is built. The 10-phase
...
- [ ] Create schema `restaurant_settlement`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `restaurant_settlement.outbox` and `restaurant_settlement.inbox` for reliable eventing
...
- [ ] Sync dependencies: ledger-service, payment-service
...
|-------|-------|---------|--------------|
| `merchant.settlement.accrued` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `merchant.payout.completed` | derived from name | see INTEGRATION.md | see INTEGRATION.md |

### Events Consumed
| Event | Producer | Handler |
|-------|----------|---------|
| `food.payment.completed` | see INTEGRATION.md | see INTEGRATION.md |
| `merchant.suspended` | see INTEGRATION.md | see INTEGRATION.md |
