# checkout-service — Technology Profile

> One-page technology reference for `checkout-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `checkout` (no partitioning — short-lived sessions, pruned)
- **Migrations**: Flyway 11.x
- **ORM / DSL**: jOOQ 3.20.x (writes) + Hibernate 7 (reads)

## 4. Cache

Redis — idempotency keys, distributed lock

## 5. External integrations

pricing · payment · ledger · tax (saga participants)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile checkout up`
...
|---|---|---|---|---|---|---|---|
| Checkout aggregate | ✓ | ✓ | ✓ | read+reason | ✓ | — | ✓ |
| Pricing / tax breakdown | ✓ | ✓ | ✓ | read+reason | ✓ | — | ✓ |
| Payment reference (PII) | ✓ | ✓ | scrubbed | scrubbed+reason | scrubbed | — | scrubbed |
...
|---|---|---|---|
| `POST` | `/admin/v1/checkouts/{id}/force-state` | `checkout.admin` | Force the checkout state machine to a specific state (with warning event) |
| `POST` | `/admin/v1/checkouts/{id}/saga-replay` | `checkout.admin` | Replay a stuck saga step (e.g. pricing → payment → ledger) |
| `GET` | `/admin/v1/checkouts/{id}/saga-trace` | `checkout.support` | Full saga trace with per-step timing and downstream state |
...
- This service's per-service admin role `checkout.admin`.
...
emit `audit.admin.checkout.v1` (per §10.2) for each touched
record.
...
**External vendor SDK.** pricing · payment · ledger · tax (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §7 for the per-service index).

**Per-service OSS libraries.** This service pulls in the full pinned set listed in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §3 *Kotlin / Spring Boot OSS dependencies* (the `platform-spring-boot-starter` for Kotlin, the standard Go stack for Go, or the FastAPI + Pydantic + SQLAlchemy set for Python). The most service-specific entries are: jOOQ 3.20 · Spring Statemachine 5 · MapStruct.
