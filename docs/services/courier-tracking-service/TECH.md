# courier-tracking-service — Technology Profile

> One-page technology reference for `courier-tracking-service`. The platform-wide
...
| **Language** | Go 1.25.x |
| **Framework** | `net/http` + WebSocket |
| **Build** | `go build` (Go 1.25.x toolchain) |
| **Container** | `gcr.io/distroless/static-debian12:nonroot` (multi-stage build, static binary) |

## 2. Key libraries

- `coder/websocket` — WebSocket fan-out
- `go-redis/redis` v9 (Redis GEO) — courier geo index
- `prometheus/client_golang` v1.20+ — metrics

## 3. Data layer

- **Database**: PostgreSQL 18, schema — (write-through to `driver-location`; courier-specific state in Redis)
- **Migrations**: n/a
- **ORM / DSL**: n/a (no own DB)

## 4. Cache

Redis — courier GEO index, last-N trail
...
- **HPA signal**: RPS, 3–30 replicas, p99 < 5ms
- **Pod resources**: requests/limits set per service (see `k8s/base/<service>/deployment.yaml`)

## 9. Local dev

- **Run**: `go run ./cmd/courier-tracking`
- **Test**: `go test ./...`
- **Compose profile**: `docker compose --profile courier-tracking up`
...
- **Key**: `audit.admin.courier_tracking.v1`
...
|---|---|---|---|---|---|---|---|
| Courier geo state (Redis) | ✓ | ✓ | ✓ | — | — | — | — |
| Last-N trail (downsampled) | ✓ | ✓ | ✓ | — | — | — | ✓ |
...
|---|---|---|---|
| `POST` | `/admin/v1/couriers/{id}/geo-resync` | `courier_tracking.admin` | Force a courier's geo index to re-sync from the database of record |
| `POST` | `/admin/v1/connections/{courier_id}/kick` | `platform.admin` | Force-close a WebSocket connection (e.g. stuck sessions) |
...
- **Run with admin port**: `ADMIN_PORT=8081 go run ./cmd/courier-tracking`
...
- This service's per-service admin role `courier_tracking.admin`.
...
emit `audit.admin.courier_tracking.v1` (per §10.2) for each touched
record.
