...
> | Documentation completeness | filesystem scan (`docs/services/api-gateway/`) |
...
| Field | Value |
|---|---|
| Service name (kebab-case) | `api-gateway` |
| Bounded context | **Edge / cross-cutting request handling.** In scope: edge security, |
| Domain | Platform Foundation |
| Tier (deployment) | 0 (position 4 of 21; `DEPLOYMENT_ORDER.md` §2) |
| Criticality / SLO | T1 (99.95% SLO) |
| Owner team | — |
...
| DB schema | `—` (per-service) | `services/README.md` env-var table |
| Cache | Redis — rate-limit counters, JWKS cache, revocation set | `TECH.md` §4 |
| HPA signal | RPS, 5–100, p99 < 5ms | `TECH.md` §8 |
| Replicas (default) | — | `TECH.md` §8 |
| p99 latency target | — | `TECH.md` §8 |
| Image | `registry.trips-enjoy.com/api-gateway:<sha>` | `README.md` §18 |
| Container port | 8080 | `TECH.md` §1 |
| Health endpoints | `/actuator/health/liveness`, `/actuator/health/readiness` | `TECH.md` §7 |
| `.env.example` | `apps/api-gateway/.env.example` ✅ | filesystem |
...
|---|---|
| Status | ✅ Graduated |
| `apps/api-gateway/` Dockerfile | ✅ present |
| `apps/api-gateway/k8s/` (flat kustomize overlays) | — |
| `apps/api-gateway/monitoring/` (ServiceMonitor + PrometheusRule) | — |
| Local test suite | (see §8.2 row above) |
| Implementation memory | see `uber-api-gateway-implementation-*.md` in project memory index |
...
| Field | Count / Value |
|---|---|
| Inbound REST APIs | 5 (full contract in INTEGRATION.md §1) |
| Outbound REST APIs | 4 (INTEGRATION.md §2) |
| Produced events | 4 (INTEGRATION.md §3) |
| Consumed events | 4 (INTEGRATION.md §4) |
...
| AuthZ | RBAC; admin role `api-gateway.admin` per `TECH.md` §10 |
...
| Time-bounded alias | `platform-internal` realm `service-claims` scope mappers (per identity-service per-service claim contract); `api-gateway.scopes` / `api-gateway.level` / `api-gateway.tenant` claims available |
...
| Field | Value |
|---|---|
| Plan header | Domain: Platform Foundation / Tier: 0 / DB Schema: `—` / Cache: Redis — rate-limit counters, JWKS cache, revocation set / HPA: RPS, 5–100, p99 < 5ms |
...
| Plan task total | (count of `T-api-gateway`-prefixed rows in `PLAN.md`) |
...
- **Implementation memory** (graduates only): `uber-api-gateway-implementation-<date>.md` (project memory index)
