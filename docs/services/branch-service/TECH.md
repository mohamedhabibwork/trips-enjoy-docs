# branch-service — Technology Profile

> One-page technology reference for `branch-service`. The platform-wide
...
- Spring Data JPA (Hibernate 7) — adjacency-list tree
- Spring Cache (Caffeine + Redis)
- Flyway 11

## 3. Data layer

- **Database**: PostgreSQL 18, schema `branch` (no partitioning — bounded by business count, not high-volume append)
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — country/region tree (TTL 24h)
...
- **HPA signal**: CPU 60%, 2–5 replicas, p99 < 100ms
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile branch up`
...
|---|---|---|---|---|---|---|---|
| Country / region / city tree | ✓ | ✓ | ✓ | read+reason | — | — | ✓ |
| Time-zone / locale metadata | ✓ | ✓ | ✓ | read+reason | — | — | ✓ |
...
|---|---|---|---|
| `POST` | `/admin/v1/branches/{id}/refresh-cache` | `branch.admin` | Invalidate the cached tree under a node and force a refresh from DB |
...
- This service's per-service admin role `branch.admin`.
...
emit `audit.admin.branch.v1` (per §10.2) for each touched
record.
