# restaurant-order-mgmt-service — Technology Profile

> One-page technology reference for `restaurant-order-mgmt-service`. The platform-wide
...
- Spring Statemachine 5 (kitchen ticket lifecycle)
- MapStruct
- Flyway 11

## 3. Data layer

- **Database**: PostgreSQL 18, schema `restaurant_order_mgmt` (no partitioning — queue items short-lived, aggressively pruned)
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — kitchen ticket state mirror
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile restaurant-order-mgmt up`
...
- `restaurant_order_mgmt.support`

### 10.2 Audit log

Every admin call on this service emits one event to:

- **Key**: `audit.admin.restaurant_order_mgmt.v1`
...
|---|---|---|---|---|---|---|---|
| Kitchen tickets | ✓ | ✓ | ✓ | read+reason | — | — | read |
| Restaurant refs | ✓ | ✓ | ✓ | read+reason | — | — | read |
...
|---|---|---|---|
| `POST` | `/admin/v1/tickets/{id}/force-state` | `restaurant_order_mgmt.admin` | Force a kitchen ticket to a target state (e.g. mark ready for an order the kitchen forgot) |
| `POST` | `/admin/v1/tickets/{id}/reassign` | `restaurant_order_mgmt.admin` | Reassign a ticket to a different kitchen station |
...
- This service's per-service admin role `restaurant_order_mgmt.admin`.
...
emit `audit.admin.restaurant_order_mgmt.v1` (per §10.2) for each touched
record.
