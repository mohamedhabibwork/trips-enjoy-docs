# restaurant-order-mgmt-service — Implementation Plan

**Domain:** Food Marketplace
**Tier:** 5
**Technology:** Kotlin/Spring
**Criticality:** T2 (99.9%)
**DB Schema:** `restaurant_order_mgmt`
**Cache:** Redis — KDS hot
...
This PLAN.md is the source of truth for **how** `restaurant-order-mgmt-service` is built. The 10-phase
...
- [ ] Create schema `restaurant_order_mgmt`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `restaurant_order_mgmt.outbox` and `restaurant_order_mgmt.inbox` for reliable eventing
...
- [ ] Sync dependencies: food-order-service, menu-service, restaurant-service
...
|-------|-------|---------|--------------|
| `food.order.preparing` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `food.order.ready` | derived from name | see INTEGRATION.md | see INTEGRATION.md |

### Events Consumed
| Event | Producer | Handler |
|-------|----------|---------|
| `food.order.placed` | see INTEGRATION.md | see INTEGRATION.md |
