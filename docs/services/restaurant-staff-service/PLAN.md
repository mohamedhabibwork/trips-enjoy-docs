# restaurant-staff-service — Implementation Plan

**Domain:** Food Marketplace
**Tier:** 4
**Technology:** Kotlin/Spring
**Criticality:** T3 (99.5%)
**DB Schema:** `restaurant_staff`
**Cache:** Redis — staff session
...
This PLAN.md is the source of truth for **how** `restaurant-staff-service` is built. The 10-phase
...
- [ ] Create schema `restaurant_staff`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `restaurant_staff.outbox` and `restaurant_staff.inbox` for reliable eventing
...
- [ ] Sync dependencies: identity-service, restaurant-service
...
|-------|-------|---------|--------------|
| `staff.invited` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `staff.activated` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `staff.deactivated` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
