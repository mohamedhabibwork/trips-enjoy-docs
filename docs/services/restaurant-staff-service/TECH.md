# restaurant-staff-service — Technology Profile

> One-page technology reference for `restaurant-staff-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `restaurant_staff` (no partitioning — bounded by staff count, not append-heavy)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile restaurant-staff up`
...
- `restaurant_staff.support`

### 10.2 Audit log

Every admin call on this service emits one event to:

- **Key**: `audit.admin.restaurant_staff.v1`
...
|---|---|---|---|---|---|---|---|
| Staff profile | ✓ | ✓ | ✓ | read+reason | — | — | scrubbed |
| Shift schedule | ✓ | ✓ | ✓ | read+reason | — | — | read |
...
|---|---|---|---|
| `POST` | `/admin/v1/staff/{id}/suspend` | `restaurant_staff.admin` | Suspend a staff member (immediate) |
| `POST` | `/admin/v1/staff/{id}/unsuspend` | `restaurant_staff.admin` | Unsuspend a staff member |
...
- This service's per-service admin role `restaurant_staff.admin`.
...
emit `audit.admin.restaurant_staff.v1` (per §10.2) for each touched
record.
