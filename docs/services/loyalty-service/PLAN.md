# loyalty-service — Implementation Plan
...
**Phase 4.** Coordinate start with the platform team.

This PLAN.md is the source of truth for **how** `loyalty-service` is built. The 10-phase
...
- [ ] Create schema `loyalty`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `loyalty.outbox` and `loyalty.inbox` for reliable eventing
...
- [ ] Sync dependencies: customer-service, trip-service, food-order-service
...
**Frequent zones aggregator.** GET /v1/accounts/{customer_id}/frequent-zones?window_days=30.
- Debounce: debounced daily.
...
| `trip-service` | per `INTEGRATION.md` | sync dependency | Yes |
| `food-order-service` | per `INTEGRATION.md` | sync dependency | Yes |

### Events Published
| Event | Topic | Trigger | Key Consumers |
|-------|-------|---------|--------------|
| `loyalty.points.earned` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `loyalty.tier.changed` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `loyalty.frequent_zone.aggregated.v1` | derived from name | see INTEGRATION.md | see INTEGRATION.md |

### Events Consumed
| Event | Producer | Handler |
|-------|----------|---------|
| `trip.completed` | see INTEGRATION.md | see INTEGRATION.md |
| `food.order.completed` | see INTEGRATION.md | see INTEGRATION.md |
