# loyalty-service — Technology Profile

> One-page technology reference for `loyalty-service`. The platform-wide
...
- Spring Statemachine 5 (tier transitions)
- MapStruct
- Flyway 11

## 3. Data layer

- **Database**: PostgreSQL 18, schema `loyalty` (monthly RANGE partitioned on `created_at`)
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — points balance (TTL 1m, push-invalidate on earn/burn)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile loyalty up`
...
|---|---|---|---|---|---|---|---|
| Points balances | ✓ | ✓ | read | read+reason | — | — | ✓ |
| Tier memberships | ✓ | ✓ | read | read+reason | — | — | read |
...
|---|---|---|---|
| `POST` | `/admin/v1/loyalty/{user_id}/adjust-points` | `loyalty.admin` | Adjust points for a user (earn / burn / correction; with audit) |
| `POST` | `/admin/v1/loyalty/{user_id}/force-tier` | `loyalty.admin` | Force-promote / force-demote a user to a specific tier |
...
- This service's per-service admin role `loyalty.admin`.
...
emit `audit.admin.loyalty.v1` (per §10.2) for each touched
record.
