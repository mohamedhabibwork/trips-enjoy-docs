# trip-service — Technology Profile

> One-page technology reference for `trip-service`. The platform-wide
...
- Spring Statemachine 5 (trip lifecycle)
- MapStruct
- Flyway 11

## 3. Data layer

- **Database**: PostgreSQL 19, schema `trip`
...
- **Per `ERD.md` 3**: tables include `trips`, `trip_stops`,
  ...
  with `REVOKE UPDATE, DELETE` from the application role — mirrors
  `ledger.postings` per the accounting four-layer truth model).
- The reward config cache is hydrated by `configuration.updated.v1`
  (DEGRADABLE upstream); a synchronous read covers the cold-start path.

## 4. Cache

Redis — active trip state mirror
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile trip up`
...
|---|---|---|---|---|---|---|---|
| Trip aggregate | ✓ | ✓ | ✓ | read+reason | — | — | scrubbed |
| Driver / customer refs | ✓ | ✓ | scrubbed | scrubbed+reason | — | — | scrubbed |
...
|---|---|---|---|
| `POST` | `/admin/v1/trips/{id}/force-state` | `trip.admin` | Force the trip state machine to a target state (e.g. complete a stuck trip) |
| `POST` | `/admin/v1/trips/{id}/reassign` | `trip.admin` | Reassign a trip's driver (operational override) |
| `POST` | `/admin/v1/trips/{id}/reward/re-evaluate` | `pricing.admin` | Force re-evaluation of the reward (e.g. after config update); emits a new `trip.reward.granted.v1` |
| `POST` | `/admin/v1/trips/{id}/reward/reverse` | `pricing.admin` | Reverse a previously-granted reward; emits `trip.reward.reversed.v1`; required `reason` ≥ 8 chars; idempotent on `Idempotency-Key` |
...
- This service's per-service admin role `trip.admin`.
