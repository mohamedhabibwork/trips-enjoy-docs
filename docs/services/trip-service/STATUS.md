...
> | Documentation completeness | filesystem scan (`docs/services/trip-service/`) |
...
| Field | Value |
|---|---|
| Service name (kebab-case) | `trip-service` |
| Bounded context | **Trip Aggregate**. |
| Domain | Ride-Hailing |
| Tier (deployment) | 2 (position 17 of 21; `DEPLOYMENT_ORDER.md` §2) |
...
| DB schema | `trip` (per-service) | `services/README.md` env-var table |
| Cache | Redis — active trip state | `TECH.md` §4 |
| HPA signal | CPU 70%, 3–8, p99 < 80ms | `TECH.md` §8 |
| Replicas (default) | — | `TECH.md` §8 |
| p99 latency target | — | `TECH.md` §8 |
| Image | `registry.trips-enjoy.com/trip-service:<sha>` | `README.md` §18 |
| Container port | 8080 | `TECH.md` §1 |
| Health endpoints | `/actuator/health/liveness`, `/actuator/health/readiness` | `TECH.md` §7 |
| `.env.example` | `apps/trip-service/.env.example` ✅ | filesystem |
...
|---|---|
| Status | ⏳ Stub |
| `apps/trip-service/` Dockerfile | — |
| `apps/trip-service/k8s/` (flat kustomize overlays) | — |
| `apps/trip-service/monitoring/` (ServiceMonitor + PrometheusRule) | — |
...
| Outbound REST APIs | 11 (INTEGRATION.md §2) |
| Produced events | 7 (INTEGRATION.md §3) |
...
| AuthZ | RBAC; admin role `trip-service.admin` per `TECH.md` §10 |
...
| Time-bounded alias | `platform-internal` realm `service-claims` scope mappers (per identity-service per-service claim contract); `trip-service.scopes` / `trip-service.level` / `trip-service.tenant` claims available |
...
| Field | Value |
|---|---|
| Plan header | Domain: Ride-Hailing / Tier: 4 / DB Schema: `trip` / Cache: Redis — active trip state / HPA: CPU 70%, 3–8, p99 < 80ms |
...
| Plan task total | (count of `T-trip-service`-prefixed rows in `PLAN.md`) |
...
- **Implementation memory** (graduates only): `uber-trip-service-implementation-<date>.md` (project memory index)
