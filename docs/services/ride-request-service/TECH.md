# ride-request-service — Technology Profile

> One-page technology reference for `ride-request-service`. The platform-wide
...
- Spring Statemachine 5 (booking lifecycle)
- MapStruct
- Flyway 11

## 3. Data layer

- **Database**: PostgreSQL 18, schema `ride_request`
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — quote hold, idempotency keys

## 5. External integrations

pricing (quote) · dispatch (matching)
...
- **HPA signal**: CPU 60%, 3–20 replicas, p99 < 200ms
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile ride-request up`
...
- `platform.support`
- `ride_request.admin`
- `ride_request.support`

### 10.2 Audit log

Every admin call on this service emits one event to:

- **Key**: `audit.admin.ride_request.v1`
...
|---|---|---|---|---|---|---|---|
| Ride request | ✓ | ✓ | ✓ | read+reason | — | — | scrubbed |
| PII (pickup, dropoff) | ✓ | ✓ | scrubbed | scrubbed+reason | — | — | scrubbed |
...
|---|---|---|---|
| `POST` | `/admin/v1/ride-requests/{id}/force-state` | `ride_request.admin` | Force the booking state machine to a target state (with warning event) |
| `POST` | `/admin/v1/ride-requests/{id}/reassign` | `ride_request.admin` | Reassign a ride request to a specific driver (operational override) |
...
- This service's per-service admin role `ride_request.admin`.
...
emit `audit.admin.ride_request.v1` (per §10.2) for each touched
record.
...
**External vendor SDK.** pricing · dispatch (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §7 for the per-service index).
...
and `dispatch.deal.bid.expired.v1` (see
[`../../shared/DEAL_FEATURE.md`](../../shared/DEAL_FEATURE.md) §4 for
the deal event catalog).

**Idempotency.** `Idempotency-Key: deal:<deal_id>:<action>` on
every state-changing POST; consumer-side in inbox dedup on
`event_id` per
[`architecture/FAILURE_HANDLING.md`](../../architecture/FAILURE_HANDLING.md).
The deal-timer sweeper uses `SELECT … FOR UPDATE SKIP LOCKED` (same
primitive as `scheduled-ride-service`).
