# driver-incentive-service — Technology Profile

> One-page technology reference for `driver-incentive-service`. The platform-wide
...
- Pydantic 2
- NumPy 2 (vectorised math)
- Pandas 2 (cohort / A-B analysis)
- `aiokafka`

## 3. Data layer

- **Database**: PostgreSQL 18, schema `driver_incentive` (monthly RANGE partitioned on `created_at`)
...
- **Test**: `pytest`
- **Compose profile**: `docker compose --profile driver-incentive up`
...
- **Key**: `audit.admin.driver_incentive.v1`
...
|---|---|---|---|
| `POST` | `/admin/v1/bonuses/{id}/force-expire` | `driver_incentive.admin` | Force-expire a bonus (e.g. budget exhausted early) |
| `POST` | `/admin/v1/cohorts/{id}/recompute` | `driver_incentive.admin` | Recompute a cohort's earnings / incentive numbers |
...
- This service's per-service admin role `driver_incentive.admin`.
...
emit `audit.admin.driver_incentive.v1` (per §10.2) for each touched
record.
