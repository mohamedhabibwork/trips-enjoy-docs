# zone-service — Implementation Plan
...
**DB Schema:** `zone`
**Cache:** Redis — zone polygon cache
...
This PLAN.md is the source of truth for **how** `zone-service` is built. The 10-phase
...
- [ ] Create schema `zone`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `zone.outbox` and `zone.inbox` for reliable eventing
...
|-------|-------|---------|--------------|
| `zone.updated` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `zone.surge.updated` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
