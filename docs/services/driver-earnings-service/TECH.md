# driver-earnings-service — Technology Profile

> One-page technology reference for `driver-earnings-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `driver_earnings`
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile driver-earnings up`
...
- `driver_earnings.finance`

### 10.2 Audit log

Every admin call on this service emits one event to:

- **Key**: `audit.admin.driver_earnings.v1`
...
|---|---|---|---|
| `POST` | `/admin/v1/earnings/{driver_id}/recompute` | `driver_earnings.admin` | Recompute earnings for a driver in a date range (after a dispute) |
...
- This service's per-service admin role `driver_earnings.admin`.
...
emit `audit.admin.driver_earnings.v1` (per §10.2) for each touched
record.
