# driver-earnings-service — Implementation Plan

**Domain:** Ride-Hailing
**Tier:** 4
**Technology:** Kotlin/Spring
**Criticality:** T2 (99.9%)
**DB Schema:** `driver_earnings`
**Cache:** Redis — period totals
**HPA:** CPU 60%, 2–4, p99 < 150ms
...
This PLAN.md is the source of truth for **how** `driver-earnings-service` is built. The 10-phase
...
- [ ] Create schema `driver_earnings`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `driver_earnings.outbox` and `driver_earnings.inbox` for reliable eventing
...
- [ ] Sync dependencies: payment-service, wallet-service
...
**Driver earnings consumer.** GET /v1/drivers/{id}/period-eligible-earnings?window=hourly|daily.
- type=guaranteed_topup on grant; type=correction on reversal.
...
|-------|-------|---------|--------------|
| `driver.earning.accrued` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `driver.withdrawal.completed` | derived from name | see INTEGRATION.md | see INTEGRATION.md |

### Events Consumed
| Event | Producer | Handler |
|-------|----------|---------|
| `ride.payment.completed` | see INTEGRATION.md | see INTEGRATION.md |
