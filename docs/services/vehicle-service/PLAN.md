# vehicle-service — Implementation Plan
...
This PLAN.md is the source of truth for **how** `vehicle-service` is built. The 10-phase
...
- [ ] Create schema `vehicle`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `vehicle.outbox` and `vehicle.inbox` for reliable eventing
...
|-------|-------|---------|--------------|
| `vehicle.registered` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `vehicle.approved` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `vehicle.insurance.expired` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
