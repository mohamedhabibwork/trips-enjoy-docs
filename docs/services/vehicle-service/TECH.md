# vehicle-service — Technology Profile

> One-page technology reference for `vehicle-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `vehicle` (no partitioning — bounded by vehicle count, identity-keyed)
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — vehicle (TTL 5m)

## 5. External integrations

file (vehicle photos, registration scans)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile vehicle up`
...
|---|---|---|---|---|---|---|---|
| Vehicle record | ✓ | ✓ | ✓ | read+reason | — | — | scrubbed |
| Document refs | ✓ | ✓ | ✓ | — | — | — | — |
...
|---|---|---|---|
| `POST` | `/admin/v1/vehicles/{id}/documents/verify` | `vehicle.admin` | Mark a vehicle document (registration, insurance, inspection) as verified |
| `POST` | `/admin/v1/vehicles/{id}/force-off-duty` | `platform.admin` | Force a vehicle off-duty (e.g. expired insurance) |
...
- This service's per-service admin role `vehicle.admin`.
...
emit `audit.admin.vehicle.v1` (per §10.2) for each touched
record.
...
**External vendor SDK.** file (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §7 for the per-service index).
