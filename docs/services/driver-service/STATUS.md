# driver-service — Status Snapshot
...
> | Documentation completeness | filesystem scan (`docs/services/driver-service/`) |
...
| Field | Value |
|---|---|
| Service name (kebab-case) | `driver-service` |
| Bounded context | **Driver profile + KYC.** In scope: driver profile, KYC |
| Domain | Identity & User |
| Tier (deployment) | 1 (position 10 of 21; `DEPLOYMENT_ORDER.md` §2) |
...
| DB schema | `driver` (per-service) | `services/README.md` env-var table |
| Cache | Redis — driver profile | `TECH.md` §4 |
| HPA signal | CPU 60%, 2–5, p99 < 80ms | `TECH.md` §8 |
| Replicas (default) | — | `TECH.md` §8 |
| p99 latency target | — | `TECH.md` §8 |
| Image | `registry.trips-enjoy.com/driver-service:<sha>` | `README.md` §18 |
| Container port | 8080 | `TECH.md` §1 |
| Health endpoints | `/actuator/health/liveness`, `/actuator/health/readiness` | `TECH.md` §7 |
| `.env.example` | `apps/driver-service/.env.example` ✅ | filesystem |
...
| `apps/driver-service/k8s/` (flat kustomize overlays) | — |
| `apps/driver-service/monitoring/` (ServiceMonitor + PrometheusRule) | — |
...
| Outbound REST APIs | 8 (INTEGRATION.md §2) |
| Produced events | 10 (INTEGRATION.md §3) |
...
| AuthZ | RBAC; admin role `driver-service.admin` per `TECH.md` §10 |
...
| Time-bounded alias | `platform-internal` realm `service-claims` scope mappers (per identity-service per-service claim contract); `driver-service.scopes` / `driver-service.level` / `driver-service.tenant` claims available |
...
| Field | Value |
|---|---|
| Plan header | Domain: Identity & User / Tier: 2 / DB Schema: `driver` / Cache: Redis — driver profile / HPA: CPU 60%, 2–5, p99 < 80ms |
...
| Plan task total | (count of `T-driver-service`-prefixed rows in `PLAN.md`) |
...
- **Implementation memory** (graduates only): `uber-driver-service-implementation-<date>.md` (project memory index)
