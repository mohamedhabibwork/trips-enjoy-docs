# driver-service — Technology Profile

> One-page technology reference for `driver-service`. The platform-wide
...
- **Database**: PostgreSQL 19, schema `driver`
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — driver profile (TTL 5m)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile driver up`
...
|---|---|---|---|---|---|---|---|
| Driver profile | ✓ | ✓ | ✓ | read+reason | — | — | scrubbed |
| Vehicle refs | ✓ | ✓ | ✓ | read+reason | — | — | scrubbed |
| PII (name, phone) | ✓ | ✓ | scrubbed | scrubbed+reason | — | — | scrubbed |
...
|---|---|---|---|
| `POST` | `/admin/v1/drivers/{id}/suspend` | `platform.admin` | Suspend a driver (immediate; emits `driver.suspended.v1`) |
| `POST` | `/admin/v1/drivers/{id}/unsuspend` | `platform.admin` | Unsuspend a driver |
| `POST` | `/admin/v1/drivers/{id}/documents/verify` | `driver.admin` | Mark a driver document (license, insurance) as verified |
...
- This service's per-service admin role `driver.admin`.
...
emit `audit.admin.driver.v1` (per 10.2) for each touched
record.
