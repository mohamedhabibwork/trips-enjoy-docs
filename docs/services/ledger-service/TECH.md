# ledger-service — Technology Profile

> One-page technology reference for `ledger-service`. The platform-wide
...
- jOOQ 3.20.x (type-safe SQL, append-only writes)
- Spring Data JPA (Hibernate 7) — read models
- MapStruct

## 3. Data layer

- **Database**: PostgreSQL 19, schema `ledger` (monthly RANGE partitioned on `posted_at`)
- **DB extras**: monthly partitions; pre-create 12 future months, BigDecimal-only money
- **Migrations**: Flyway 11.x
- **ORM / DSL**: jOOQ 3.20.x (writes) + Hibernate 7 (reads)

## 4. Cache

— (no caching on money — read from DB)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile ledger up`
...
|---|---|---|---|---|---|---|---|
| Chart of accounts | ✓ | ✓ | read | — | ✓ | — | read |
| Ledger entries (money) | ✓ | ✓ | — | — | ✓ | — | read |
| Reconciliation status | ✓ | ✓ | read | — | ✓ | — | read |
...
|---|---|---|---|
| `POST` | `/admin/v1/ledger/reconciliation/run` | `ledger.admin` | Trigger a reconciliation run against providers / banks / wallets |
| `GET` | `/admin/v1/ledger/accounts/{id}/balance` | `ledger.finance` | Balance for a specific account at a specific point in time (incl. holds) |
| `POST` | `/admin/v1/ledger/accounts/{id}/adjust` | `ledger.admin` | Issue a balancing adjustment (with double-entry pairing; auto-balanced) |
...
- This service's per-service admin role `ledger.admin`.
...
emit `audit.admin.ledger.v1` (per 10.2) for each touched
record.
