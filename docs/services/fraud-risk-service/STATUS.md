# fraud-risk-service — Status Snapshot
...
> | Documentation completeness | filesystem scan (`docs/services/fraud-risk-service/`) |
...
| Field | Value |
|---|---|
| Service name (kebab-case) | `fraud-risk-service` |
| Bounded context | **Bounded Context**: *Fraud detection / risk scoring*. |
| Domain | Platform Support |
| Tier (deployment) | 1 (position 14 of 21; `DEPLOYMENT_ORDER.md` §2) |
...
| DB schema | `fraud_risk` (per-service) | `services/README.md` env-var table |
| Cache | Redis — feature cache | `TECH.md` §4 |
| HPA signal | CPU 70%, 2–6, p99 < 150ms | `TECH.md` §8 |
| Replicas (default) | — | `TECH.md` §8 |
| p99 latency target | — | `TECH.md` §8 |
| Image | `registry.trips-enjoy.com/fraud-risk-service:<sha>` | `README.md` §18 |
| Container port | 8080 | `TECH.md` §1 |
| Health endpoints | `/actuator/health/liveness`, `/actuator/health/readiness` | `TECH.md` §7 |
| `.env.example` | `apps/fraud-risk-service/.env.example` ✅ | filesystem |
...
|---|---|
| Status | ⏳ Stub |
| `apps/fraud-risk-service/` Dockerfile | — |
| `apps/fraud-risk-service/k8s/` (flat kustomize overlays) | — |
| `apps/fraud-risk-service/monitoring/` (ServiceMonitor + PrometheusRule) | — |
...
| `SKELETON.pyproject.toml` | ✅ mandatory | ✅ | 2026-08-12 |
...
| Outbound REST APIs | 6 (INTEGRATION.md §2) |
...
| AuthZ | RBAC; admin role `fraud-risk-service.admin` per `TECH.md` §10 |
...
| Time-bounded alias | `platform-internal` realm `service-claims` scope mappers (per identity-service per-service claim contract); `fraud-risk-service.scopes` / `fraud-risk-service.level` / `fraud-risk-service.tenant` claims available |
...
| Field | Value |
|---|---|
| Plan header | Domain: Platform Support / Tier: 2 / DB Schema: `fraud_risk` / Cache: Redis — feature cache / HPA: CPU 70%, 2–6, p99 < 150ms |
...
| Plan task total | (count of `T-fraud-risk-service`-prefixed rows in `PLAN.md`) |
...
- **Implementation memory** (graduates only): `uber-fraud-risk-service-implementation-<date>.md` (project memory index)
