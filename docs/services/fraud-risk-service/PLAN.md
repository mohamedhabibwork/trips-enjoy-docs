...
- [ ] Create schema `fraud_risk`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `fraud_risk.outbox` and `fraud_risk.inbox` for reliable eventing
