# ride-safety-service — Technology Profile

> One-page technology reference for `ride-safety-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `ride_safety` (monthly RANGE partitioned on `reported_at` / `recorded_at`)
...
Redis — active trip-share sessions

## 5. External integrations

communication-gateway (emergency contacts) · file (recording chunks)
...
- **HPA signal**: CPU 60%, 2–5 replicas, p99 < 300ms
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile ride-safety up`
...
- **Key**: `audit.admin.ride_safety.v1`
...
|---|---|---|---|---|---|---|---|
| Trip-share sessions | ✓ | ✓ | ✓ | read+reason | — | — | scrubbed |
| Emergency contacts | ✓ | ✓ | scrubbed | scrubbed+reason | — | — | scrubbed |
| Recording refs | ✓ | ✓ | — | — | — | — | — |
...
|---|---|---|---|
| `POST` | `/admin/v1/trip-shares/{id}/revoke` | `ride_safety.admin` | Revoke a trip-share session (e.g. after a safety incident) |
| `GET` | `/admin/v1/trips/{trip_id}/safety-log` | `platform.support` | Full safety log for a trip (incl. emergency triggers, recording flags) |
...
- This service's per-service admin role `ride_safety.admin`.
...
emit `audit.admin.ride_safety.v1` (per §10.2) for each touched
record.
...
**External vendor SDK.** communication-gateway · file (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §7 for the per-service index).
