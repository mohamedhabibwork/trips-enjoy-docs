# ride-payment-integration-service — Technology Profile

> One-page technology reference for `ride-payment-integration-service`. The platform-wide
...
- jOOQ 3.20.x (type-safe SQL)
- Spring Statemachine 5 (saga state)
- Spring Data JPA (Hibernate 7) — read models

## 3. Data layer

- **Database**: PostgreSQL 18, schema `ride_payment_integration` (monthly RANGE partitioned on `occurred_at`)
...
Redis — saga state, idempotency keys
...
- **HPA signal**: CPU 60%, 3–15 replicas, p99 < 500ms
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile ride-payment-integration up`
...
- `ride_payment_integration.finance`

### 10.2 Audit log

Every admin call on this service emits one event to:

- **Key**: `audit.admin.ride_payment_integration.v1`
...
|---|---|---|---|---|---|---|---|
| Saga state | ✓ | ✓ | read | — | ✓ | — | read |
| Money (BigDecimal) | ✓ | ✓ | — | — | ✓ | — | read |
...
|---|---|---|---|
| `POST` | `/admin/v1/sagas/{saga_id}/replay` | `ride_payment_integration.admin` | Replay a stuck saga step (e.g. payment → wallet → ledger) |
| `GET` | `/admin/v1/sagas/{saga_id}/trace` | `ride_payment_integration.admin` | Full saga trace with per-step timing and downstream state |
| `POST` | `/admin/v1/sagas/{saga_id}/force-state` | `ride_payment_integration.admin` | Force the saga state machine to a target state (with warning event) |
...
- This service's per-service admin role `ride_payment_integration.admin`.
...
emit `audit.admin.ride_payment_integration.v1` (per §10.2) for each touched
record.
...
**External vendor SDK.** payment · wallet · ledger (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §7 for the per-service index).

**Per-service OSS libraries.** This service pulls in the full pinned set listed in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §3 *Kotlin / Spring Boot OSS dependencies* (the `platform-spring-boot-starter` for Kotlin, the standard Go stack for Go, or the FastAPI + Pydantic + SQLAlchemy set for Python). The most service-specific entries are: jOOQ 3.20 · Spring Statemachine 5 · Spring Data JPA (read).
