# ride-payment-integration-service — Implementation Plan

**Domain:** Ride-Hailing
**Tier:** 5
**Technology:** Kotlin/Spring
**Criticality:** T1 (99.95%)
**DB Schema:** `ride_payment_integration`
**Cache:** Redis — saga state
...
**Phase 3 — Ride-Hailing.** The ride-payment saga depends on trip + payment + driver-earnings.

This PLAN.md is the source of truth for **how** `ride-payment-integration-service` is built. The 10-phase
...
- [ ] Create schema `ride_payment_integration`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `ride_payment_integration.outbox` and `ride_payment_integration.inbox` for reliable eventing
...
- [ ] Sync dependencies: payment-service, wallet-service, ledger-service, pricing-service, driver-earnings-service
...
| `wallet-service` | per `INTEGRATION.md` | sync dependency | Yes |
| `ledger-service` | per `INTEGRATION.md` | sync dependency | Yes |
| `pricing-service` | per `INTEGRATION.md` | sync dependency | Yes |
| `driver-earnings-service` | per `INTEGRATION.md` | sync dependency | Yes |

### Events Published
| Event | Topic | Trigger | Key Consumers |
|-------|-------|---------|--------------|
| `ride.payment.completed` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `ride.payment.failed` | derived from name | see INTEGRATION.md | see INTEGRATION.md |

### Events Consumed
| Event | Producer | Handler |
|-------|----------|---------|
| `trip.completed` | see INTEGRATION.md | see INTEGRATION.md |
| `payment.authorized` | see INTEGRATION.md | see INTEGRATION.md |
