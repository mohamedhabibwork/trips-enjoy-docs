...
> | Documentation completeness | filesystem scan (`docs/services/file-service/`) |
...
| Field | Value |
|---|---|
| Service name (kebab-case) | `file-service` |
| Bounded context | **Bounded Context**: *File / media storage abstraction* |
| Domain | Platform Foundation |
| Tier (deployment) | 0 (position 5 of 21; `DEPLOYMENT_ORDER.md` §2) |
| Criticality / SLO | T2 (99.9%) |
| Owner team | — |
...
| DB schema | `file` (per-service) | `services/README.md` env-var table |
| Cache | Redis — presigned URL cache | `TECH.md` §4 |
| HPA signal | CPU 60%, 2–4, p99 < 200ms | `TECH.md` §8 |
| Replicas (default) | — | `TECH.md` §8 |
| p99 latency target | — | `TECH.md` §8 |
| Image | `registry.trips-enjoy.com/file-service:<sha>` | `README.md` §18 |
| Container port | 8080 | `TECH.md` §1 |
| Health endpoints | `/actuator/health/liveness`, `/actuator/health/readiness` | `TECH.md` §7 |
| `.env.example` | `apps/file-service/.env.example` ✅ | filesystem |
...
|---|---|
| Status | ✅ Graduated |
| `apps/file-service/` Dockerfile | ✅ present |
| `apps/file-service/k8s/` (flat kustomize overlays) | ✅ present |
| `apps/file-service/monitoring/` (ServiceMonitor + PrometheusRule) | ✅ present |
| Local test suite | (see §8.2 row above) |
| Implementation memory | see `uber-file-service-implementation-*.md` in project memory index |
...
| Field | Count / Value |
|---|---|
| Inbound REST APIs | 14 (full contract in INTEGRATION.md §1) |
...
| AuthZ | RBAC; admin role `file-service.admin` per `TECH.md` §10 |
...
| Time-bounded alias | `platform-internal` realm `service-claims` scope mappers (per identity-service per-service claim contract); `file-service.scopes` / `file-service.level` / `file-service.tenant` claims available |
...
| Field | Value |
|---|---|
| Plan header | Domain: Platform Foundation / Tier: 1 / DB Schema: `file` / Cache: Redis — presigned URL cache / HPA: CPU 60%, 2–4, p99 < 200ms |
...
| Plan task total | (count of `T-file-service`-prefixed rows in `PLAN.md`) |
...
- **Implementation memory** (graduates only): `uber-file-service-implementation-<date>.md` (project memory index)
