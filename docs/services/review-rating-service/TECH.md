# review-rating-service — Technology Profile

> One-page technology reference for `review-rating-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `review_rating` (monthly RANGE partitioned on `created_at`)
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — aggregate ratings (TTL 1h)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile review-rating up`
...
- `platform.support`
- `review_rating.admin`
- `review_rating.support`

### 10.2 Audit log

Every admin call on this service emits one event to:

- **Key**: `audit.admin.review_rating.v1`
...
|---|---|---|---|---|---|---|---|
| Reviews / ratings | ✓ | ✓ | read | read+reason | — | — | scrubbed |
...
|---|---|---|---|
| `POST` | `/admin/v1/reviews/{id}/moderate` | `review_rating.admin` | Moderate a review (remove / restore / flag) with audit + reason |
| `POST` | `/admin/v1/reviews/{id}/respond` | `review_rating.admin` | Post an official response on behalf of the platform |
...
- This service's per-service admin role `review_rating.admin`.
...
emit `audit.admin.review_rating.v1` (per §10.2) for each touched
record.
