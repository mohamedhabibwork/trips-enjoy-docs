# review-rating-service — Implementation Plan

**Domain:** Analytics
**Tier:** 4
**Technology:** Kotlin/Spring
**Criticality:** T2 (99.9%)
**DB Schema:** `review_rating`
**Cache:** Redis — aggregate window
...
This PLAN.md is the source of truth for **how** `review-rating-service` is built. The 10-phase
...
- [ ] Create schema `review_rating`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `review_rating.outbox` and `review_rating.inbox` for reliable eventing
...
**Zone-aggregated driver rating.** GET /v1/zones/{zone_id}/driver-rating?window_minutes=15.
- Debounce: debounced per zone.
...
|-------|-------|---------|--------------|
| `review.submitted` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `review.aggregated` | derived from name | see INTEGRATION.md | see INTEGRATION.md |
| `review.zone_aggregated.v1` | derived from name | see INTEGRATION.md | see INTEGRATION.md |

### Events Consumed
| Event | Producer | Handler |
|-------|----------|---------|
| `trip.completed` | see INTEGRATION.md | see INTEGRATION.md |
| `food.order.delivered` | see INTEGRATION.md | see INTEGRATION.md |
