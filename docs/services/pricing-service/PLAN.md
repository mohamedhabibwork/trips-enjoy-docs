...
- [ ] Create schema `pricing`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `pricing.outbox` and `pricing.inbox` for reliable eventing
...
### Phase 5 — Caching
- [ ] Redis — quote cache + rule snapshot
