# food-order-service — Technology Profile

> One-page technology reference for `food-order-service`. The platform-wide
...
- Spring Statemachine 5 (order lifecycle)
- MapStruct
- Flyway 11

## 3. Data layer

- **Database**: PostgreSQL 19, schema `food_order` (monthly RANGE partitioned on `placed_at`)
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — cart/order dedup window
...
- **HPA signal**: CPU 60%, 2–10 replicas, p99 < 300ms
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile food-order up`
...
|---|---|---|---|---|---|---|---|
| Order aggregate | ✓ | ✓ | ✓ | read+reason | — | — | ✓ |
| Customer refs | ✓ | ✓ | scrubbed | scrubbed+reason | — | — | scrubbed |
...
|---|---|---|---|
| `POST` | `/admin/v1/food-orders/{id}/force-state` | `food_order.admin` | Force the order state machine to a target state (e.g. cancel a stuck order) |
| `POST` | `/admin/v1/food-orders/{id}/refund` | `platform.finance` | Issue an out-of-band refund (links to `payment-service` + `ledger-service`) |
...
- This service's per-service admin role `food_order.admin`.
...
emit `audit.admin.food_order.v1` (per 10.2) for each touched
record.
...
**External vendor SDK.** restaurant · menu (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) 7 for the per-service index).
...
once a deal moves to `matched`. The fare band is computed against
the order's `total_minor` (from the `pricing-service` quote) and
backed by the same `deal.band.{tenant}.{city}.{ride_type}.{…}`
keys (where `ride_type` is overloaded to `delivery_type` for the
food vertical — `courier`, `scooter`, `bicycle`).

**Events.** Produces `food.deal.opened.v1`, `food.deal.countered.v1`,
`food.deal.accepted.v1`, `food.deal.rejected.v1`,
`food.deal.expired.v1`. Consumes
`delivery.deal.bid.submitted.v1` and
`delivery.deal.bid.expired.v1` (see
