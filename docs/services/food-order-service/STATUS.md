# food-order-service — Status Snapshot
...
> | Documentation completeness | filesystem scan (`docs/services/food-order-service/`) |
...
| Field | Value |
|---|---|
| Service name (kebab-case) | `food-order-service` |
| Bounded context | - **In scope**: food order state machine, configuration |
| Domain | Food Marketplace |
| Tier (deployment) | 2 (position 18 of 21; `DEPLOYMENT_ORDER.md` §2) |
...
| DB schema | `food_order` (per-service) | `services/README.md` env-var table |
| Cache | Redis — active order | `TECH.md` §4 |
| HPA signal | CPU 60%, 2–6, p99 < 150ms | `TECH.md` §8 |
| Replicas (default) | — | `TECH.md` §8 |
| p99 latency target | — | `TECH.md` §8 |
| Image | `registry.trips-enjoy.com/food-order-service:<sha>` | `README.md` §18 |
| Container port | 8080 | `TECH.md` §1 |
| Health endpoints | `/actuator/health/liveness`, `/actuator/health/readiness` | `TECH.md` §7 |
| `.env.example` | `apps/food-order-service/.env.example` ✅ | filesystem |
...
|---|---|
| Status | ⏳ Stub |
| `apps/food-order-service/` Dockerfile | — |
| `apps/food-order-service/k8s/` (flat kustomize overlays) | — |
| `apps/food-order-service/monitoring/` (ServiceMonitor + PrometheusRule) | — |
...
| Produced events | 11 (INTEGRATION.md §3) |
...
| AuthZ | RBAC; admin role `food-order-service.admin` per `TECH.md` §10 |
...
| Time-bounded alias | `platform-internal` realm `service-claims` scope mappers (per identity-service per-service claim contract); `food-order-service.scopes` / `food-order-service.level` / `food-order-service.tenant` claims available |
...
| Field | Value |
|---|---|
| Plan header | Domain: Food Marketplace / Tier: 5 / DB Schema: `food_order` / Cache: Redis — active order / HPA: CPU 60%, 2–6, p99 < 150ms |
...
| Plan task total | (count of `T-food-order-service`-prefixed rows in `PLAN.md`) |
...
- **Implementation memory** (graduates only): `uber-food-order-service-implementation-<date>.md` (project memory index)
