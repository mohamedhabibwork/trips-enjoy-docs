...
- [ ] Create schema `food_order`: tables per `ERD.md` (partitioned by time/zone/hash per data shape)
...
- [ ] Add `food_order.outbox` and `food_order.inbox` for reliable eventing
