# analytics-service — Technology Profile

> One-page technology reference for `analytics-service`. The platform-wide
...
- Spring Kafka 4 (exactly-once + DLQ)
- Apicurio / Confluent schema registry (Avro)
- AWS SDK v2 (S3)
- Snowflake JDBC

## 3. Data layer

- **Database**: PostgreSQL 18, schema `analytics` (monthly RANGE partitioned on `created_at`)
- **DB extras**: monthly partitions; pre-create 12 future months
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Spring Data JPA (Hibernate 7) — read-only metadata
...
S3 (Parquet data lake) · Snowflake / BigQuery / Redshift (OLAP)
...
- **HPA signal**: Kafka consumer lag, 3–15 replicas, 50k evt/s
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile analytics up`
...
|---|---|---|---|
| `POST` | `/admin/v1/pipeline/{topic}/reset-offset` | `analytics.admin` | Reset a topic's consumer offset to a specific timestamp or ID |
| `POST` | `/admin/v1/pipeline/{topic}/replay` | `analytics.admin` | Replay a topic's events from a given offset into S3 / warehouse |
| `GET` | `/admin/v1/pipeline/lag` | `platform.engineering` | Current Kafka consumer lag per topic-partition |
...
- This service's per-service admin role `analytics.admin`.
...
emit `audit.admin.analytics.v1` (per §10.2) for each touched
record.
...
**Profile context.** Streaming / event ingest — Kotlin / Spring Boot 4 + Spring Kafka.

**External vendor SDK.** S3 · Snowflake/BigQuery/Redshift (see the entry in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §7 for the per-service index).

**Per-service OSS libraries.** This service pulls in the full pinned set listed in [`../../shared/OSS_DEPENDENCIES.md`](../../shared/OSS_DEPENDENCIES.md) §3 *Kotlin / Spring Boot OSS dependencies* (the `platform-spring-boot-starter` for Kotlin, the standard Go stack for Go, or the FastAPI + Pydantic + SQLAlchemy set for Python). The most service-specific entries are: Spring Kafka 4 · Apicurio (Avro) · AWS SDK v2 (S3) · Snowflake JDBC.
