# address-service — Technology Profile

> One-page technology reference for `address-service`. The platform-wide
...
- **Database**: PostgreSQL 18, schema `address` (no partitioning — low volume per identity, identity-keyed access)
- **Migrations**: Flyway 11.x
- **ORM / DSL**: Hibernate 7 (Spring Data JPA)

## 4. Cache

Redis — by-user address list (TTL 1h)
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile address up`
...
|---|---|---|---|---|---|---|---|
| Addresses (own data) | ✓ | ✓ | ✓ | read+reason | — | — | ✓ |
| PII (street, lat/lon) | ✓ | ✓ | scrubbed | scrubbed+reason | — | — | scrubbed |
...
|---|---|---|---|
| `GET` | `/admin/v1/addresses/{user_id}` | `platform.support` | List all addresses for a user — PII scrubbed unless `?reason=...` |
| `POST` | `/admin/v1/addresses/{id}/geocode-refresh` | `address.admin` | Force re-geocode an address (e.g. after map-provider data update) |
...
- This service's per-service admin role `address.admin`.
...
emit `audit.admin.address.v1` (per §10.2) for each touched
record.
