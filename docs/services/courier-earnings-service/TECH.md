# courier-earnings-service — Technology Profile

> One-page technology reference for `courier-earnings-service`. The platform-wide
...
- Spring Batch (period statements)
- MapStruct
- Flyway 11

## 3. Data layer

- **Database**: PostgreSQL 18, schema `courier_earnings` (monthly RANGE partitioned on `accrued_at`)
...
- **HPA signal**: CPU 60%, 2–5 replicas, p99 < 1s
...
- **Run**: ./gradlew bootRun
- **Test**: ./gradlew test
- **Compose profile**: `docker compose --profile courier-earnings up`
...
- `courier_earnings.finance`

### 10.2 Audit log

Every admin call on this service emits one event to:

- **Key**: `audit.admin.courier_earnings.v1`
...
|---|---|---|---|---|---|---|---|
| Earnings aggregates | ✓ | ✓ | read | — | ✓ | — | ✓ |
| Payout history | ✓ | ✓ | read | — | ✓ | — | read |
...
|---|---|---|---|
| `POST` | `/admin/v1/earnings/{courier_id}/recompute` | `courier_earnings.admin` | Recompute earnings for a courier in a date range (after a dispute) |
...
- This service's per-service admin role `courier_earnings.admin`.
...
emit `audit.admin.courier_earnings.v1` (per §10.2) for each touched
record.
