# customer-service — Status Snapshot
...
> | Documentation completeness | filesystem scan (`docs/services/customer-service/`) |
...
| Bounded context | **Customer profile.** In scope: customer profile (KYC, LTV, |
| Domain | Identity & User |
| Tier (deployment) | 1 (position 9 of 21; `DEPLOYMENT_ORDER.md` §2) |
...
| DB schema | `customer` (per-service) | `services/README.md` env-var table |
| Cache | Redis — customer + segment | `TECH.md` §4 |
| HPA signal | CPU 60%, 2–5, p99 < 80ms | `TECH.md` §8 |
| Replicas (default) | — | `TECH.md` §8 |
| p99 latency target | — | `TECH.md` §8 |
| Image | `registry.trips-enjoy.com/customer-service:<sha>` | `README.md` §18 |
| Container port | 8080 | `TECH.md` §1 |
| Health endpoints | `/actuator/health/liveness`, `/actuator/health/readiness` | `TECH.md` §7 |
| `.env.example` | `apps/customer-service/.env.example` ✅ | filesystem |
...
| `apps/customer-service/k8s/` (flat kustomize overlays) | — |
| `apps/customer-service/monitoring/` (ServiceMonitor + PrometheusRule) | — |
...
| Consumed events | 11 (INTEGRATION.md §4) |
...
| AuthZ | RBAC; admin role `customer-service.admin` per `TECH.md` §10 |
...
| Time-bounded alias | `platform-internal` realm `service-claims` scope mappers (per identity-service per-service claim contract); `customer-service.scopes` / `customer-service.level` / `customer-service.tenant` claims available |
...
| Field | Value |
|---|---|
| Plan header | Domain: Identity & User / Tier: 2 / DB Schema: `customer` / Cache: Redis — customer + segment / HPA: CPU 60%, 2–5, p99 < 80ms |
...
| Plan task total | (count of `T-customer-service`-prefixed rows in `PLAN.md`) |
...
- **Implementation memory** (graduates only): `uber-customer-service-implementation-<date>.md` (project memory index)
