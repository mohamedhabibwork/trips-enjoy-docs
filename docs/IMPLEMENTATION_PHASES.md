...
> **Purpose:** Week-by-week implementation roadmap for all 58 services
...
- [ ] **communication-gateway-service** - Multi-channel messaging
- [ ] **file-service** - File storage

### Week 3-4: Audit & Zones
- [ ] **audit-service** - Audit logging
- [ ] **zone-service** - Service zones
...
- [ ] **user-profile-service** - User preferences
...
- [ ] **support-service** - Support ticketing
- [ ] **fraud-risk-service** - Risk scoring
- [ ] **promotion-service** - Promotions & coupons
...
- [ ] **driver-availability-service** - Driver online status
- [ ] **driver-location-service** - Location tracking
- [ ] **eta-routing-service** - ETA calculations

### Week 15-17: Ride Flow
- [ ] **ride-request-service** - Ride requests
- [ ] **dispatch-service** - Ride dispatch
- [ ] **trip-service** - Trip management

### Week 18-19: Ride Payment & Earnings
- [ ] **ride-payment-integration-service** - Ride payment saga
- [ ] **driver-earnings-service** - Driver earnings

### Week 19-20: Ride Enhancements
- [ ] **scheduled-ride-service** - Scheduled rides
- [ ] **ride-safety-service** - Safety features
- [ ] **ride-history-service** - Historical trips
- [ ] **driver-incentive-service** - Driver bonuses
...
- [ ] **branch-service** - Restaurant branches
- [ ] **restaurant-staff-service** - Staff management
...
- [ ] **restaurant-order-mgmt-service** - Kitchen order management
...
### Week 29-31: Delivery
- [ ] **courier-dispatch-service** - Courier matching
- [ ] **courier-tracking-service** - Courier location tracking
- [ ] **delivery-service** - Delivery orchestration

### Week 32-34: Financial Settlement
- [ ] **food-payment-integration-service** - Food payment saga
- [ ] **restaurant-settlement-service** - Merchant payouts
- [ ] **courier-earnings-service** - Courier earnings
...
- [ ] **analytics-service** - Data warehouse ingestion
- [ ] **reporting-service** - BI dashboards
- [ ] **ride-history-service** (if not done in Phase 3)
...
- [ ] **`feature-flag-service`** — surface `deal.enabled.{city_id}.{ride_type}`.
- [ ] **`audit-service`** — consume all `*.deal.*.v1` and write `audit.deal_transition.v1`.

### Week 42: Ride + dispatch + food

- [ ] **`ride-request-service`** — rider-side boundary; add `POST /v1/rides/{id}/deal`, `POST /v1/deals/{id}/counter|accept|reject`, `GET /v1/deals/{id}`. Produce `ride.deal.opened.v1`, `ride.deal.countered.v1`, `ride.deal.accepted.v1`, `ride.deal.rejected.v1`, `ride.deal.expired.v1`. Consume `dispatch.deal.bid.submitted.v1`, `dispatch.deal.bid.expired.v1`, `dispatch.deal.accepted.v1`.
- [ ] **`dispatch-service`** — driver-side boundary; add `GET /v1/dispatch/drivers/{id}/open-deals`, `POST /v1/dispatch/deals/{id}/bids|accept`, `POST /v1/dispatch/deals/{id}/bid/{bid_id}/reject`. Produce `dispatch.deal.bid.submitted.v1`, `dispatch.deal.bid.expired.v1`, `dispatch.deal.accepted.v1`, `dispatch.deal.rejected.v1`. Consume `ride.deal.opened.v1`, `ride.deal.countered.v1`, `ride.deal.accepted.v1`, `ride.deal.rejected.v1`, `ride.deal.expired.v1`.
- [ ] **`food-order-service`** — customer-side boundary; add `POST /v1/orders/{id}/deal` and the deal endpoints. Produce `food.deal.*.v1`. Consume `delivery.deal.bid.submitted.v1` (delivered by `courier-dispatch-service`).
- [ ] **`courier-dispatch-service`** — courier-side boundary; mirror the dispatch-service changes for the food vertical.
...
- [x] **driver-earnings-service** — consume the grant as `type=guaranteed_topup` and the reversal as `type=correction`; new `GET /v1/drivers/{id}/period-eligible-earnings?window=hourly|daily` endpoint for `trip-service`.
- [x] **wallet-service** — consume the user-side grant (idempotency-key `trip:{trip_id}:reward:user:grant`) and credit/debit the customer wallet.
...
- [x] **review-rating-service** — new `GET /v1/zones/{zone_id}/driver-rating?window_minutes=15` and `review.zone_aggregated.v1` event (debounced per zone).
- [x] **loyalty-service** — new `GET /v1/accounts/{customer_id}/frequent-zones?window_days=30` and `loyalty.frequent_zone.aggregated.v1` event (debounced daily).
